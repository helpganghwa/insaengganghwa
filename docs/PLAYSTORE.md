...
- **복구**(`lib/payment/play-recover.ts`, `recoverPlayPurchases`): 앱에서 상점을 열 때 한 번 기기의 `listPurchases()`를 서버로 보내 다시 검증·지급·소모한다. 매칭은 토큰이 묶인 주문 → 같은 SKU의 7일 내 미완 주문 → SKU로 되돌린 새 주문(성장패스 구간은 불가). 서버가 구글에 확인해 **취소됨(CANCELLED)** 이라고 답한 구매만 기기에서 `consume()`해 잠김을 푼다(보류 결제는 건드리지 않는다 — 대개 화면의 첫 검증이 토큰을 주문에 묶고, 아니어도 결제가 끝나면 RTDN·다음 복구가 처리한다). 같은 SKU 미완 주문은 토큰 없는 것 중 마지막 결제 시도 순으로 고른다. **다른 게임 계정 보호**: `listPurchases()`는 구글 계정 단위라 같은 기기에서 게임 계정을 바꾸면 남의 구매가 보인다. 그래서 토큰이 다른 유저 주문에 묶였거나, 다른 유저가 구매 직전([구매−30분, 구매+5분], 주문을 [만든 시각, 마지막 결제 시도] 구간으로 봄)에 연 토큰 없는 미완 주문이 있거나, 구글이 구매 시각을 주지 않으면 지급하지 않고 `PLAY_RTDN_UNMATCHED`(`recover:<구글 주문>`, 구매당 1회) 경보만 남긴다. 기기는 소모하지 않으므로 운영자가 어드민 도구로 지급하고, 3일 안에 처리하지 않으면 구글이 자동 환불한다. 그동안 그 구글 계정은 같은 상품을 다시 살 수 없다. 한계: 진짜 구매자가 결제창을 연 뒤 30분 넘게 지나 구매를 확정하면 이 보호에 걸리지 않는다. 30분은 오지급 위험과 정상 복구 차단 빈도를 맞바꾸는 값이다.
