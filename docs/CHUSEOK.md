...
- 3만 이후는 같은 보상을 반복한다(더 모으면 한 번 더). 누적 기준이라 추가 시점에 이미 넘긴 분도 바로 받는다.
...
- 5만·7만·10만 추가분 추정(9/26 실측, 9/30까지): 도달 5만 22~34명·7만 16~25명·10만 9~17명 → 💎38만~61만·📦1.4만~2.3만. 추가 즉시 소급 도달 7·5·3명(💎12만·📦4,500).
...
1. **병합**: `git merge --no-ff --no-edit origin/master`(Play 결제 핫픽스 4건, 충돌 0 확인·트리 동일) → `bun run typecheck && bun run build`(빌드가 아틀라스·asset-versions를 다시 쓰므로 `git status`로 변경 0 확인) → master-dev 푸시 → 스테이징 health `dpl` 변경 확인(health는 CDN 30초 캐시 — `curl "…/api/health?_=$(date +%s)"`).
...
6. **9/24 00:00~00:10**: 보급 풀·확률 공시 42종(2.38%) 확인, 00:07까지 `probability_snapshots` 새 행(effective_at 9/24 00:00 KST, note "추석 6종 개방", `jsonb_typeof='object'`)과 `catalog_items` 6종 `active=true` 확인(이 크론 경로는 스테이징에서 SQL로 켜서 실DB로 돈 적이 없음 — 개방 실패 시 운영 경보가 옴). 없으면 수동 절차: ① 프로덕션에서 `update catalog_items set active=true where code like 'chuseok_%' and active=false` ② 어드민으로 로그인한 탭 콘솔에서 `await fetch('/api/admin/revalidate?tag=catalog',{method:'POST'}).then(r=>r.json())`(CRON_SECRET은 로컬에 없음) ③ **스크립트는 스테이징을 기본으로 읽으므로 반드시 덮어쓰기**: `DIRECT_URL="$(grep '^PROD_DATABASE_URL=' .env.local | cut -d= -f2- | tr -d '"')" bun run scripts/record-probability-snapshot.ts`로 드라이런 42/42/42 확인 뒤 `--confirm --note="추석 6종 개방"`(effective_at은 실행 시각으로 찍힘). revalidate 직후 첫 요청은 옛 값일 수 있어 두 번 새로고침.
...
**롤백**: 인스턴트 롤백(`vercel rollback <dpl> --yes`, 상태 `vercel rollback status`) → health `dpl` 확인(필요 시 `vercel promote <dpl> --yes`) → **새 코드가 살아 있는 동안은 active=false로 6종이 숨겨지지 않는다**(시각 게이트) → 9/24 00:02 이후라면 `update catalog_items set active=false where code like 'chuseok_%'` + catalog 태그 revalidate(옛 아틀라스에 그림이 없어 이미지가 깨짐). 0207·0209 적용 전이어야 안전하다. 롤백 중 옛 코드는 마일리지를 profiles.mileage에만 쌓으므로 표시가 어긋나며, 다시 배포할 때 0211을 한 번 더 돌린다. 배포 시점에 열려 있던 클라이언트는 채팅 실시간 토픽이 바뀌어 새로고침 전까지 60초 폴링만 받는다(데이터 영향 없음).
