...
> | **2단계 출시** | 출시 전날 밤~당일 아침 | 크론 정지 → **`maintenance` ON**(가드 통과 + 어드민·심사 동시 쓰기 차단 — cbt_ended는 이 둘을 안 막는다) → wipe(2차, **`--keep-payments`** — 테스트 실결제 보존) → §3.5 복원(캐릭터+보상 지급) → **`cbt_ended` 복원**(오픈 전까지 종료 화면 유지) → 크론 재개 → §4 env·§4.5 완화값·§5 확률공시·§6 서버명 → 오픈 시각에 `system_mode='live'` → 오픈 공지 + 오픈 푸시(서버 실행 경로) |
>
