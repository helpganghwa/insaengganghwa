...
>
> **이번 출시의 날짜·확정값·실행 명령은 `docs/LAUNCH-PLAN-2026-08.md`가 정본** — 본 런북은 절차의 일반형이다.
>
> **2단계 플로우(2026-07-31 확정)** — 종료일과 출시일 사이에 테스트 기간(약 9일)이 있다:
>
> | 단계 | 시점 | 하는 일 |
> |---|---|---|
> | **1단계 종료·동결** | CBT 종료일 밤 | 결산 수치 갱신·배포 → 크론 정지 → **`maintenance` ON**(스크립트 가드 요구 + wipe 중 동시 쓰기 차단) → §2 스냅샷 → pg_dump → §3 wipe(1차) → `system_mode='cbt_ended'` → 크론 재개. **§3.5 복원은 하지 않는다** |
> | 테스트 기간 | 종료~출시 전 | cbt_ended 상태 — 어드민·심사 계정만 접속. 카드사 심사 대응·결제 E2E·기능 점검. 테스트 흔적은 2차 wipe가 지우므로 자유롭게 |
> | **2단계 출시** | 출시 전날 밤~당일 아침 | 크론 정지 → **`maintenance` ON**(가드 통과 + 어드민·심사 동시 쓰기 차단 — cbt_ended는 이 둘을 안 막는다) → wipe(2차, **`--keep-payments`** — 테스트 실결제 보존) → §3.5 복원(캐릭터+보상 지급) → **`cbt_ended` 복원**(오픈 전까지 종료 화면 유지) → 크론 재개 → §4 env·§4.5 완화값·§5 확률공시·§6 서버명 → 오픈 시각에 `system_mode='live'` → 오픈 공지 + `open-push-broadcast` |
>
...
>
> **출시일 아침 크론 안전(2026-07-31 점검)** — 대난투(09:00 KST)·일일 보급 푸시(새벽)가 오픈(11:00) 전에 돌면 유령 진행·잠긴 유저 대상 푸시가 생긴다. `push-daily-supply`·`melee-run`·`melee-reveal`은 **cbt_ended 동안 크론 내부 게이트로 자동 skip** — 2단계 작업 후 크론을 밤새 켜둬도 안전하고, 11:00 `live` 전환 순간부터 정상 동작한다(첫 대난투는 오픈 다음날 09:00 — 로스터가 '강화 1회+' 기준이라 오픈일 아침엔 어차피 no-op이지만 게이트가 이중 방어). 나머지 크론(점령·연대기·정산류)은 빈 월드에서 자연 no-op.
