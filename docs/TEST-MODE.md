# 테스트 모드 — 보상 ×10 및 원복 절차

실운영(정식 출시) 전 테스트 기간 동안 일부 보상을 ×10로 지급한다. 본 문서는 그 범위와
...
테스트 기간 ×10가 적용되는 보상은 **3종뿐**이다.

| 보상 | 정상값 | 테스트값(×10) | 적용 메커니즘 |
|------|--------|--------------|--------------|
| 신규가입 보너스 — 다이아 | 💎 1,000 | 💎 10,000 | DB 트리거 (`handle_new_user`) |
| 신규가입 보너스 — 보급상자 | 슬롯당 10개 | 슬롯당 100개 | DB 트리거 (`handle_new_user`) |
| 출석체크 보상 | ×1 | ×10 | TS 상수 `TEST_REWARD_MULTIPLIER` |
| 일일 우편 보급 | 💎 1,000 + 슬롯당 5장 | 💎 10,000 + 슬롯당 50장 | TS 상수 `TEST_REWARD_MULTIPLIER` |
