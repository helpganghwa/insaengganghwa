...
| S9 | **회수 부족** — 취소는 됐는데 유저가 이미 소비해 회수액이 모자람 | 高 | 인라인(환불 tx) |
...
| S6 | 알림 | 초과분 환불 처리 |
| S9 | 부족분 기록(`clawback_done=false`) | 채권 회수/제재 판단, 재발 시 S3 연계 |

### 6.1 환불 심사 절차

구현은 **전액환불 + 잔액 회수만** 지원(부분환불 없음). 환불정책 §1("사용하지 않은 유상분 기준")은
코드가 집행한다 — 회수 가능액을 먼저 계산하고, 모자라면 취소 자체를 시작하지 않는다.

**어드민 환불(우리가 취소를 일으키는 유일한 지점) = 사전 차단**

1. 환불 요청 접수 → 어드민 결제 화면에서 해당 주문의 "환불".
2. `previewClawback(userId, serverId, productCode)`이 지급량 vs 현재 잔액을 대조.
   부족하면 **포트원 취소를 호출하지 않고** 거부 — 결제는 그대로 유지된다.
   거부 문구가 부족 내역(지급/보유)을 그대로 보여주므로 유저 안내에 그대로 쓴다.
3. 충분하면 PG 전액취소 → `refundPurchase`가 재화 회수 + `iap_refunds` 기록.
4. 운영 판단으로 그래도 환불해야 하면(민원·법적 요구 등) **강제 환불**: 체크박스 + 사유 입력 2단계.
   사유는 `admin_actions`(`payment.refund.force`)에 부족 내역과 함께 남는다.
5. 반복 환불 어뷰징 유저는 S3 절차(결제 차단 검토)로 연계.

**웹훅·recon(PG에서 이미 취소가 끝난 뒤) = 사후 기록**

막을 수 있는 시점이 지났으므로 회수는 가능한 만큼 수행하고, 부족분을
`iap_refunds.clawback_done=false` + `REFUND_CLAWBACK_SHORT` 알림(S9)으로 남긴다.
여기서 회수를 거부하면 "환불도 되고 재화도 남는" 최악이 된다.

`REFUND_CLAWBACK_SHORT`는 **재시도 대상이 아니다** — 기계적 실패가 아니라 소비 사실이라
다시 돌려도 결과가 같다. 운영자 판단(채권 회수·제재)으로 종결한다.

부분환불(소비분 비례 차감 환불)은 출시 후 필요 시 구현 — v1에서는 위 절차가 정본.
