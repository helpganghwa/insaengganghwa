...
### 6.1 환불 심사 절차 (v1 확정 — 2026-07-10)

구현은 **전액환불 + 잔액 회수만** 지원(부분환불 없음, `reclaimProductGrant`가 GREATEST(0) 클램프
회수 — 이미 소비한 분은 회수 불가·손실 처리). 환불정책 문구("미사용 유상분 기준")는 아래
**어드민 수동 심사**로 집행한다:

1. 환불 요청 접수 → 어드민 결제 화면에서 해당 주문의 지급량 vs 현재 잔액 확인.
2. **소비분이 유의미하면**(지급 다이아의 상당분을 이미 사용) 정책 근거로 **환불 거절** —
   "디지털 콘텐츠 사용 개시 후 청약철회 제한"(약관·환불정책 고지 근거).
3. 미사용(잔액 ≥ 지급량)이면 PG 전액취소 → 자동 회수(웹훅/recon이 잔여 처리).
4. 반복 환불 어뷰징 유저는 S3 절차(결제 차단 검토)로 연계.

부분환불(소비분 비례 차감 환불)은 출시 후 필요 시 구현 — v1에서는 위 수동 절차가 정본.
