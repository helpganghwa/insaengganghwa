...
알림은 단일 함수 `raisePaymentAlert(kind, opts)`로 일원화. 두 싱크:

- **영속·중복방지(항상)**: `payment_alerts` 테이블. 같은 `(kind, order_id)` 미해결 건은 1회만 생성·발송. 어드민에서 `resolved` 처리.
- **즉시 푸시(선택)**: `PAYMENT_ALERT_WEBHOOK_URL`(Discord/Slack incoming webhook)이 설정돼 있으면 `fetch` POST. 의존성 0(SDK 불필요).
- 이메일(Resend)은 영수증 발송이 실제 연동되면 세 번째 싱크로 추가(현재 Resend 미설치 — 법무 문서상 예정 수탁자).
