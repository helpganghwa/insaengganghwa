...
### 1.4 무기 스프라이트 구도 — 대각선·크게 + 두꺼운 타입
무기는 방어구·장신구보다 캔버스 점유가 낮아 작아 보인다(특히 가는 검을 세로로 두면 더 작음).
- **모든 무기 프롬프트에 "large and diagonal"** — 정사각 캔버스를 대각선으로 크게 채워 작아 보임 방지.
- **두꺼운 타입(대검·투핸드소드·할버드·워해머 등)을 적극 기용** — 지역마다 가는 무기에 치우치지 말고
  굵은 실루엣을 섞는다. (단 무기는 §1.1 규칙대로 아바타가 한 손에 쥔 정지 자세로도 멋져야 함.)
...
**120종은 전부 신규 생성한다**(기존에서 차감하지 않음). 유지 여부는 **신규 120종이 완성된 뒤**,
완성된 카탈로그를 실제로 보고 사후에 판단한다.

사후 판단 기준:
1. 단독으로도 개성 있고(세트 의존 X) 신규와 안 겹치는가
2. 새 팔레트·미감에 자연스럽게 섞이는가
3. 신규 중 약한 항목을 *교체*하거나 카탈로그에 *추가*할 가치가 있는가

→ 즉 유지 아이템은 신규 120 위에 얹는 **선택적 보너스/교체** 후보이지, 생성 전에 확정하지 않는다.

---

## 6. 다음 단계
1. **120 타입 리스트 확정** (본 문서 §3 리뷰·조정)
2. **항목별 nameKo · lore · art · wornDesc 작성** (슬롯/지역별 배치, 검토 반복)
3. **Pixellab 스프라이트 생성** — 유료, 배치 단위, **생성 전 확인 필수**
4. **신규 120종 완성 후** 기존 108종 유지 판단(§5 — 선택적 추가/교체)
