...
2. **P2 스키마**: Drizzle 스키마 + 수동 SQL(017x — expeditions·expedition_levels·부분 유니크).
   (user_vouchers는 이용권 보류로 v1 제외)
3. **P3 서버**: start/claim/cancel/reduce 액션 + RNG 롤(레벨·시너지·대성공) + 지급 트랜잭션 + 단위 테스트.
4. **P4 UI**: /expedition 화면 + 홈 카드 + 수령 팝업(대성공 연출) + 완료 푸시 + 도전 과제 3종 + 위키.
5. **P5 검수**: 적대 검수(멱등·시계·경제 상한) → 스테이징 → 배포(공시 §6 동시 공개).
