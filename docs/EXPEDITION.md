...
### 3.3 필요 강화 합 (성장축 ③ — 2026-08-27 사용자 확정: 전투력이 아닌 **강화 합**, 상한 없음)
...
- **유저 기준치(B)** = 보유 아바타 AS의 **최댓값**. 장착 3종 합이 아니라 아바타 기준인 이유: 요구를
  만족할 아바타가 최소 하나는 있어야 하고, "고강 장비로 아바타를 새로 만들 이유"(생성 sink)가 생긴다.
- **미션 필요 강화 합(R)** — 시간(4/8/12/24h)과 **독립**으로 오퍼 롤 시 확정(카드 표기):
  `R = round₁₀(B × k)`, k ∈ {0.5, 0.7, 0.9, 1.1} 균등. **1번 슬롯은 k ∈ {0.5, 0.7}**(저강 아바타도 배정
  가능). B < 30이면 R = 0(신규 보호). **상한 없음** — B가 곧 상한. k=1.1 미션은 "조금 더 강화하면 받는"
  도전 미션으로 작동.
- **배정 조건**: 선택 아바타 AS ≥ R. **시작 시점에만** 검사(진행 중 하락 무관). 미달 시 `REQ_NOT_MET`.
- **배율 M(R) = 1 + 0.9 × (R/1,000)^0.8** — 상한 없는 완만한 거듭제곱(선형보다 후반 인플레 억제).
  R 100→×1.14 · 300→×1.34 · 600→×1.60 · 1,000→×1.90 · 2,000→×2.57 · 3,000→×3.17.
  최종 = 기본 × 시간 스케일 × M(R) × (1+레벨) × (1+시너지) — bp 합산(`applyMultiplier`).
- **경제 가드 재조정**: 축 ③이 얹히므로 기본 수량을 **×0.6**(상자만 3~4 · 다이아만 72~144 · 둘 다 상자
  2~3 + 다이아 12~29). 무강화 유저 ≈ 종전 60%, 합 1,000 ≈ 114%, 합 2,000 ≈ 155%. 조절 손잡이는
  `EXPEDITION_REQ_MULT_COEF`(0.9) 하나 — 실측 후 재조정.
- 하락 리스크 연동: 현재 레벨 기준이라 강화 하락 시 AS·B가 내려가 다음 오퍼의 R도 낮아진다(자기보정).
- 저장: `expeditions.required_sum`(오퍼 시), `req_bonus_bp`(시작 시 스냅샷) — 0178.
- 공시: 3분기 확률은 불변. R의 k 분포(4구간 균등·1번 슬롯 2구간)와 M(R) 식을 공시 payload에 표기(§33).
- UI: 오퍼 카드 "필요 강화 합 R · ×M", 배정 팝업에서 미달 아바타는 흐리게 + "강화 합 AS/R", 부제에 요구치.
