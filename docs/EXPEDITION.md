...
### 3.3 아바타 강화 합 (성장축 ③ — 2026-08-27 사용자 확정: 전투력 아닌 **강화 합**, 최소치 없음, 상한 없음)
...
- **배율 M(AS) = 1 + 1.5 × (AS/1,000)^0.9** — 배정 아바타 AS만으로 결정, 상한 없음.
  100→×1.19 · 300→×1.51 · 666(출시 4일차 1위)→×2.04 · 1,000→×2.50 · 1,500→×3.16 · 2,000→×3.80.
  최종 = 기본 × 시간 스케일 × (1 + 레벨 + 시너지 + M(AS)−1 + 달성 보너스) — bp 합산(`applyMultiplier`).
- **권장 강화 합(R)** — 최소 조건이 아니라 **목표치**(최소치는 실측상 T 1,000~3,000 구간 유저의 상위 3종 합이
  ~138이라 미션 대부분이 잠겨 기각, 2026-08-27). 시간과 독립으로 오퍼 롤 시 확정(카드 표기):
  `R = round₁₀(B × k)`, B = 보유 아바타 AS 최댓값, k ∈ {0.5, 0.7, 0.9, 1.1} 균등, **1번 슬롯은 {0.5, 0.7}**,
  B < 30이면 R = 0(권장치 없음). 배정 아바타 AS ≥ R이면 **달성 보너스 +15%**. 미달 페널티 없음(시너지와 같은
  "감소 없음" 원칙). k=1.1 미션은 "조금 더 강화하면 보너스를 받는" 도전 목표.
- **경제(시뮬 2026-08-27, Lv30 풀가동 24h×3 기준)**: AS 0 → 390💎/일, 100 → 464, 300 → 588, 666 → 796,
  1,000 → 975, 1,500 → 1,233. 강화 100마다 시도 시간 2배라 1년 뒤 최상위 AS ≈ 1,300~1,500. 기본 수량은
  **×0.6**(상자만 3~4 · 다이아만 72~144 · 둘 다 상자 2~3 + 다이아 12~29). 조절 손잡이는
  `EXPEDITION_AS_MULT_COEF`(1.5) 하나 — 실측 후 재조정. 상위권일수록 즉시완료·새로고침·아바타 생성 sink 사용량도
  커서 순유입은 표보다 작다.
- 하락 리스크 연동: 현재 레벨 기준이라 강화 하락 시 AS가 내려가 배율도 내려간다(자기보정).
- 저장: `expeditions.required_sum`(오퍼 시), `req_bonus_bp`(시작 시 M(AS)−1 + 달성 보너스 스냅샷) — 0178.
- 공시: 3분기 확률은 불변. M(AS) 식, R의 k 분포(4구간 균등·1번 슬롯 2구간), 달성 보너스를 공시 payload에 표기(§33).
- UI: 오퍼 카드 "권장 강화 합 R · 달성 +15%", 배정 팝업 아바타 타일에 "강화 합 AS · ×M"과 '권장 달성' 배지,
  기본 선택은 강화 합 최대 아바타, 미리보기에 배율 합산.
