...
- **미션 슬롯**: 고정 슬롯에 서버가 **랜덤 미션(지역 × 난이도)**을 롤해서 올려둔다.
  - 슬롯 해금: 기본 1칸 · 슬롯2 = 파견 Lv.5 무료 **또는** 💎500 · 슬롯3 = Lv.15 무료 **또는** 💎1,500
    (이중 해금 — 성장 보상이자 조급함의 sink).
- **난이도 = 시간 통합**: 쉬움 4h / 보통 8h / 어려움 12h / 원정 24h(스케일 0.55/1.0/1.6/3.4).
  1h 옵션은 폐지(난이도 체계로 흡수). 출현 분포: 35/35/20/10%.
- **보상 시작 전 확정**(핵심): 미션 생성 시 서버가 본상 분기·수량·대성공까지 **미리 롤해 카드에
  확정 표기**한다(새로고침 = 보상 리롤이라 의미가 생김). ✨대성공 미션(10%)은 오퍼에서 반짝임 —
  수령 서프라이즈 대신 "새로고침 잭팟" 재미. 수령은 확정분 지급만(추가 RNG 없음).
