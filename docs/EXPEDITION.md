...
- **미션 슬롯**: 고정 슬롯에 서버가 **랜덤 미션(지역 × 난이도)**을 롤해서 올려둔다.
  - 슬롯 해금(2026-08-28 개편, 사용자 원안 확정): **4칸 전부 계정 합산 강화**(보유 장비 강화 레벨 합 = 랭킹
    '합산 강화')로만 오픈 — 슬롯1 1,000 · 슬롯2 3,000(5,000에서 하향 — 1칸 체류 구간 완화) · 슬롯3 10,000 · 슬롯4 15,000. **다이아·파견 레벨 해금 없음**
    (강화가 파견의 유일한 입구이자 규모). 합산 강화는 현재 레벨 합이라 하락 시 닫힐 수 있다 → 진행 중 파견은
    수령까지 유지, **새 배정만** 잠금(SLOT_LOCKED). 오픈 시점 분포(08-28): ≥1k 24명 · ≥3k 5명 · ≥10k 2명 · ≥15k 0명
    (활동 382명 중) — 대다수에겐 "강화하면 열린다" 목표로 노출(잠금 카드에 진행도 표시).
...
- **경제 가드**(2026-08-28 슬롯 개편 기준): 슬롯당 24h 풀가동 무강화 기대 ≈ 100💎/일 → 4칸(합산 강화 15,000+) 400💎/일, 여기에 축 ③ M(AS)·시너지가 곱해진다. 슬롯 수 자체가 강화 진척에 묶여 있어 유입 총량은 상위권에 집중(§3.3 시뮬). 시즌 후 실측으로 재조정.
