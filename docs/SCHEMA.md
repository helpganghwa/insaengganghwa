...
서버·이정표별로 **처음 넘은 세 사람**(금·은·동)을 남긴다. 최초 이정표 칭호(`first_<key>_<rank>`, docs/TITLES.md)의 판정 정본.

```sql
create table milestone_firsts (
  server_id  smallint not null,
  milestone  text not null,          -- lib/game/balance.ts FIRST_MILESTONES key (enh500 · sum20k · t30 · combat10m)
...
- 탈퇴는 명시 삭제(withdraw.ts), 빈 순위는 재충원하지 않는다.
