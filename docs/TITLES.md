...
| 상태 | 의미 | 목록 표시 | 조건 표시 |
|---|---|---|---|
| **미발견** | 한 번도 달성 못 함 | 이름만 보임(이름이 수수께끼 힌트). `hidden`은 **목록에 아예 없음** | **??? — 절대 미표시** |
| **발견** | 달성한 적 있음(원장 기록). 조건부 칭호가 지금 조건을 잃은 상태 포함 | 이름 + 최초 발견일 | 조건 공개 |
| **활성** | 지금 대표로 걸 수 있음 — 영구형은 발견=활성, 조건부형은 조건 충족 중일 때만 | 장착 버튼 활성 | 조건 공개 |

⚠ 이에 따라 **조건부 칭호도 최초 달성 시 원장에 기록한다**(발견 상태의 근거).
활성 여부만 실시간 파생으로 재검증한다 — §1의 "조건부는 저장 없음"은 이 표로 대체.
