...
async function dispatch(subs: SubRow[], payload: PushPayload): Promise<SendResult> {
  ...
  // 구독이 진짜 낡았다면 다음 배치 발송(일일 보급 등 3건 이상)이 정리하므로 보류 비용은 알림 한 번뿐이다.
  // ⚠ PushAutoSync 재구독은 정리하지 못한다 — 등록이 endpoint 단위 upsert라 새 endpoint면
  // 새 행이 생기고 옛 행은 남는다(6차 검수에서 주석 정정).
  //
