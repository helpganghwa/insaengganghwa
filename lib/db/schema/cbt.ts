 ...
 *
 * 이월 범위(정책, 2026-07-31 확정): **닉네임 + 감사 보상(합산강화 비례 💎) + 추천 보상**.
 * 아바타·진행도(강화/다이아/장비)는 이월하지 않는다 — 리셋 오픈이 전제.
 *
...
export const cbtCarryover = pgTable(
  'cbt_carryover',
  {
    ...
    /** CBT 합산강화(전 서버 enhance_level 합) — 감사 보상 산정 근거(0144). */
    totalEnhance: integer('total_enhance').notNull().default(0),
    /** 감사 보상 다이아(기본 + 합산강화 비례, 스냅샷 시점 확정 — 0144). */
    thanksDiamond: integer('thanks_diamond').notNull().default(0),
