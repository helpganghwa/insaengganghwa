-- ───────────────────────────────────────────────────────────────────────────
-- 0028 테스트 보너스 ×10 → ×5 — handle_new_user 갱신 (1회 적용)
--
-- 변경: 신규가입 다이아 10000 → 5000, 보급상자 슬롯별 100 → 50 (기본값 ×5).
-- 0007 대비 값만 변경 — 닉네임 로직(0005)·기본 프로필 시드는 그대로.
--
-- 해제: 0006 SQL 재적용 → 기본값(1000 / 10)으로 원복.
-- ───────────────────────────────────────────────────────────────────────────
...
begin
  ...
  loop
    ...
    begin
      ...
      values (new.id, new_nickname, 5000, 1);
      exit;
