-- 0064 거주지 생성 시 랜덤 배정(GUILD §5.5·SERVER.md) — 가입 트리거 v8 + 미배정 백필. 멱등.
--   캐릭터는 태어날 때 자기 서버의 구역 하나에 거주(내 위치) — lazy 배정의 빈 표시 제거.

-- 1) 미배정 캐릭터 백필 — 자기 서버의 랜덤 구역.
update characters c
set residence_zone_id = (
  select z.id from zones z
  where z.server_id = c.server_id
  order by random() limit 1
)
where c.residence_zone_id is null;

-- 2) 가입 트리거 v8 — 캐릭터 생성에 거주지 포함.
...
begin
  ...
  select id into v_zone from public.zones where server_id = v_server order by random() limit 1;
