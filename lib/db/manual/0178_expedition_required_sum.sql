...
-- 미션마다 시간과 독립으로 "권장 강화 합"(required_sum)을 롤한다(최소치 아님). 배율은 배정 아바타의
-- 강화 합(생성에 쓴 장비 3종의 현재 enhance_level 합) 곡선 + 권장치 달성 보너스이며, 시작 시
-- req_bonus_bp로 시너지·레벨과 함께 스냅샷한다(final_reward 산출 근거 보존 — 분쟁 조사).
