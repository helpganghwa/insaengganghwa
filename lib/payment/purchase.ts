...
export async function completePurchase(
  ...
  if (order.status === 'paid') {
    ...
    if (order.provider === 'play' && opts.playPurchaseToken && order.playPurchaseToken && opts.playPurchaseToken !== order.playPurchaseToken) {
      // 같은 주문으로 두 번째 구매가 성사된 경우(여러 기기에서 같은 재사용 주문) — 두 번째 구매는 아직 지급되지 않았다.
      // 조용히 거부하면 구매자가 영구 미지급으로 남으므로 경보로 드러낸다. RTDN도 이 경우는 애매함으로 경보한다.
      await raisePaymentAlert('PLAY_TOKEN_USED', {
        paymentId: `${paymentId}:${opts.playPurchaseToken.slice(0, 12)}`,
        orderId: order.id,
        detail: `이미 지급된 주문(${order.playSku})에 다른 구매 토큰이 옴 — 두 번째 구매 미지급. 상점 상품이면 다음 상점 복구가 새 주문으로 지급할 수 있으니 먼저 지급 여부 확인(어드민 도구 dryRun으로 토큰 묶임 확인, 복구가 막히면 recover: 경보가 뜬다), 1회·주기 상품·성장 프리미엄(활성 중)·성장패스면 콘솔에서 환불. 우편 등 수동 지급은 복구와 겹쳐 2중 지급이 되니 하지 말 것.`,
      });
  ...
  if (order.provider === 'play') {
    ...
    const [dup] = await db
      ...
      .limit(1);
    if (dup) {
      // 한 구매 토큰이 두 주문으로 오면 어느 쪽이든 잘못 묶였을 수 있다 — 조용히 거부하면 구매자가 영구 미지급으로
      // 남는다(2026-09-24 감사). 경보로 남겨 운영자가 콘솔과 대조한다.
      await raisePaymentAlert('PLAY_TOKEN_USED', {
        paymentId,
        orderId: order.id,
        detail: `같은 Play 구매 토큰이 다른 주문(id ${dup.id})에 이미 묶임 — 이 주문(${order.playSku}) 지급 거부. 두 주문의 유저·시각을 콘솔 주문과 대조할 것.`,
      });
      return { ok: false, code: 'TOKEN_USED' };
    }
  ...
  await db.transaction(async (tx) => {
    ...
    // 보류 결제로 **다른** 토큰이 먼저 묶인 미완 주문 — 덮어쓰면 그 보류 결제가 주인을 잃는다(재검증 B-1). 전이하지 않는다.
    if (play && locked.token && locked.token !== play.token) {
      tokenConflict = true;
      return;
    }
  ...
  if (!transitioned) {
    if (tokenConflict && play) {
      await raisePaymentAlert('PLAY_TOKEN_USED', {
        paymentId: `${paymentId}:${play.token.slice(0, 12)}`,
        orderId: order.id,
        detail: `보류 결제 토큰이 먼저 묶인 주문에 다른 구매(${play.googleOrderId ?? '?'})가 옴 — 이 구매 미지급. 상점 상품이면 다음 상점 복구가 새 주문으로 지급할 수 있으니 먼저 지급 여부 확인(어드민 도구 dryRun으로 토큰 묶임 확인, 복구가 막히면 recover: 경보가 뜬다), 1회·주기 상품·성장 프리미엄(활성 중)·성장패스면 콘솔에서 환불. 우편 등 수동 지급은 복구와 겹쳐 2중 지급이 되니 하지 말 것.`,
      });
      return { ok: false, code: 'TOKEN_USED' };
    }
    if (lockedStatus === 'paid') {
      ...
      if (play && lockedToken && lockedToken !== play.token) {
        await raisePaymentAlert('PLAY_TOKEN_USED', {
          paymentId: `${paymentId}:${play.token.slice(0, 12)}`,
          orderId: order.id,
          detail: `같은 주문에 두 구매가 동시에 옴 — 이 구매(${play.googleOrderId ?? '?'}) 미지급. 상점 상품이면 다음 상점 복구가 새 주문으로 지급할 수 있으니 먼저 지급 여부 확인(어드민 도구 dryRun으로 토큰 묶임 확인, 복구가 막히면 recover: 경보가 뜬다), 1회·주기 상품·성장 프리미엄(활성 중)·성장패스면 콘솔에서 환불. 우편 등 수동 지급은 복구와 겹쳐 2중 지급이 되니 하지 말 것.`,
        });
        return { ok: false, code: 'TOKEN_USED' };
      }
  ...
  if (dupSkipped) {
    ...
    try {
      if (play) {
        ...
        await refundPlayOrder(play.googleOrderId, true);
      } else {
      ...
      console.error('[purchase] duplicate auto refund failed', order.provider, paymentId, e);
    }
    // 자동 환불까지 끝난 건은 조치할 것이 없다 — 경보 없이 로그만('진짜 오류만' 경보 원칙).
    if (refunded) console.info(`[purchase] 중복 결제 자동 환불 완료 ${paymentId} (${order.productCode} · 서버 ${order.serverId})`);
    else await raisePaymentAlert('REFUND_RECLAIM_FAILED', {
      paymentId,
      orderId: order.id,
  ...
  if (minorExceeded) {
    ...
    const { refundPurchase } = await import('./refund');
    // 환불 API가 성공했으면 구글 상태 반영 지연을 기다리지 않는다. 실패했으면 상태 확인에 맡긴다(미취소면 NOT_CANCELLED로 빠짐).
    await refundPurchase(paymentId, { reason: 'minor_protection', playVoided: !!play && cancelled }).catch((e) =>
