 ...
 * 주문 중 구매 시각 기준 [-15분, +2분]에 만들어진 것을 찾는다 — **정확히 1건일 때만** 지급(completePurchase:
 * 구글 재검증·지급·소모, 멱등). 0건·여러 건이면 지급하지 않고 경보(다른 유저 주문에 잘못 지급하지 않기 위해).
...
export async function handleOneTimePurchase(sku: string, purchaseToken: string): Promise<RtdnOutcome> {
  ...
  const pick = pickRtdnCandidate(rows, purchaseTimeMs);
  if (!pick) {
    await raisePaymentAlert('PLAY_RTDN_UNMATCHED', {
      paymentId: `rtdn:${g.orderId ?? purchaseToken.slice(0, 16)}`,
      detail: `구글 주문 ${g.orderId ?? '?'}(${sku}) — 같은 상품의 미완 주문 ${rows.length}건(${rows.map((r) => r.paymentId).join(', ') || '없음'}). Play Console에서 확인 뒤 /api/admin/play-complete-order로 지급.`,
    });
