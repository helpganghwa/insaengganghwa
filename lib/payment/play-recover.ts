  ...
  if (!mineInWindow) {
    ...
    if (others.length > 0) {
      await raisePaymentAlert('PLAY_RTDN_UNMATCHED', {
        ...
        detail: `상점 복구: 구글 주문 ${g.orderId ?? '?'}(${sku})가 이 기기에 있지만 복구한 유저의 결제 시도가 구매 시각 근처에 없고, 다른 유저의 미완 주문(${others.map((o) => o.paymentId).join(', ')})이 있다 — 같은 기기의 다른 게임 계정 구매일 수 있어 지급하지 않음. 완료 알림(RTDN)이 처리하지 못하면 콘솔에서 구매자 확인 뒤 /api/admin/play-complete-order로 지급.`,
      });
