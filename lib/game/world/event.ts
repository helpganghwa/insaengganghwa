...
export type WorldEventType =
  ...
  | 'expedition_crit' // 파견 대성공 — detail { region, hours, diamond, boxes }. 파견 페이지 하단 전용(월드 피드·채팅 제외, 2026-08-27)
...
async function getWorldFeedUncached(serverId: number, limit = 40): Promise<WorldEventEntry[]> {
  const rows = await db
    ...
    .from(worldEvents)
    // zone_neutralized(연대기 전용)·expedition_crit(파견 페이지 전용)는 월드 피드/채팅엔 노출하지 않는다(피드 노이즈 방지).
    .where(and(eq(worldEvents.serverId, serverId), notInArray(worldEvents.type, ['zone_neutralized', 'expedition_crit'])))
