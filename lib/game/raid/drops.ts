 ...
 * 돌파 페이즈마다 보급 상자 RAID_PHASE_DROP_BOXES개 — 다이아 드롭 없음.
 *
 * ⚠ 슬롯은 **균등 분배가 아니라 박스별 해시 추첨**이다(2026-08-11 주석 정정). 페이즈당 1개라
 * 애초에 균등 분할이 불가능하고, 슬롯은 (raidId, phase, i) 해시로 정해진다 — 기대값만 1/3씩이고
 * 실제 회차는 치우친다(3페이즈 돌파 2,000건 실측: 2/1/0이 63.8%, 1/1/1은 31.4%, 3/0/0도 4.8%).
 * 같은 레이드 참가자끼리는 동일하므로 공정하고 편차는 회차 간 다양성이다. 슬롯별 공급량을
 * 계산할 때 균등으로 가정하지 말 것.
 */
import {
