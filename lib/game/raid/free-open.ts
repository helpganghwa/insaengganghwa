 ...
 */
export function raidFreeOpenActive(at: Date | number = Date.now(), env: string | undefined = process.env.VERCEL_ENV): boolean {
...
export function raidOpenCost(
  ...
  return raidFreeOpenActive(at, env) && hostedToday < RAID_FREE_OPENS_PER_DAY ? 0 : RAID_TIERS[tier].openCost;
}
