...
INPUT:
- A SINGLE image: the front (south) view of the pixel art character (~256x256 PNG).
...
FAIL the character if the front view clearly shows a defect below (e.g. crossed arms PLUS a held weapon reading as a third arm).

FAIL if any of these is clearly true in the front view:
...
- quality: a view is unrecognizable noise/garbage, OR the character is missing an essential body part (no head, no torso), OR ANATOMICALLY IMPOSSIBLE BODY-PART COUNTS — the number of a body part does not match a normal humanoid. Specifically FAIL for:
  ...
  · a WIELDED WEAPON WITH NO HAND CONTACT — the description says the weapon is held/gripped, but in the clearest FRONT view the weapon stands or floats clearly SEPARATE from the character with NO hand touching it: both hands are visibly occupied elsewhere (e.g. both resting on the hips) while the weapon stands alone beside the figure. A hand resting ON TOP of a planted weapon, cradling it in the arms, or gripping it anywhere counts as contact — FAIL only when in the front view NO hand touches the weapon at all.
  · a WIELDED WEAPON THAT IS ENTIRELY MISSING — the description says the character wields/holds a weapon, but in the front view the hands are clearly EMPTY and NO held weapon-like object appears anywhere on the character. FAIL (reason "quality"). This is the equipped weapon, not an optional accessory. Be conservative: do NOT fail if a weapon IS present but small, stylized, flaming, dagger-sized, or partly hidden behind the body/cape — FAIL only when no weapon is held in the view (both hands visibly empty).
...
- A body part hidden by the viewing angle (a turned/three-quarter pose or the body itself hides one arm or one eye). Fewer-than-normal parts due to perspective is NORMAL.
...
For anatomy: count carefully in the front view. Before failing for an extra ARM specifically, confirm it is a genuine human arm (skin/sleeve, attached at a shoulder, a hand at the end) — held weapons (especially bows) and side accessories are the most common false positives, so a limb-like object that could be a held weapon or accessory is NOT an extra arm. If something could be a pose/overlap, a held weapon, or an accessory/cloth rather than a true extra part, lean PASS. Only FAIL (reason "quality") when an extra arm/leg/head/eye is unambiguous and clearly grows from the body.
