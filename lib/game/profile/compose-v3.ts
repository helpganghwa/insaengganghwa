...
POSE: use the given pose for the arms and stance, following its mood (usually calm and composed) — natural, relaxed and loose (a gentle weight shift onto one leg is fine), staying FRONT-FACING toward the viewer (both shoulders, chest, feet and face square to the camera). Keep the hands and any weapon at shoulder height or lower, within the frame. HANDS — NAME THEM EXPLICITLY: assign every gesture to a specific hand and say which ('her right hand grips the rapier while her left arm hangs relaxed at her side') — never ambiguous references like 'the other hand', 'one hand', or 'her lower hand'. Give the free hand ONLY what the given pose says — do not invent extra gestures (no hand on the hip unless the pose itself says so). When the pose does rest a hand on the hip, state that ONLY that single hand is on the hip and that the opposite hand grips the weapon — the weapon hand always wins over the pose.
WEAPON — FAITHFUL, HIGH DETAIL, PROMINENT: the weapon is the character's SIGNATURE item and deserves the LARGEST SHARE of your description. Reproduce it faithfully and in rich detail from its IMAGE, as if lifting the actual item onto the character — preserve its exact silhouette, main colors, materials and signature ornaments so it reads as the same weapon. TYPE FIRST: when the item block gives a 'type:' line, that type IS what the weapon is — name it with that exact type word early in the weapon sentence, then describe its ornaments. A wing, feather, flame or crystal motif BELONGS TO THE WEAPON: it may shape the weapon's own parts — the blade's outline, the guard, the limbs — exactly as the image shows, and it stays part of the single object gripped in the hand. Keep that motif on the weapon alone; the character's own body and back carry only what their armor and accessory give them. Describe it PART BY PART with concrete geometry so the generator reproduces the correct proportions — for a bladed or blunt weapon: blade or head shape and its length relative to the character's body, guard shape, grip color and wrap, pommel; for a RANGED weapon: use its own anatomy words — a bow has two curved limbs joined by a taut bowstring and a wrapped center grip; a CROSSBOW has SHORT limbs set CROSSWISE on a horizontal wooden stock with a trigger under the grip and a bolt lying in the groove along the top, and it is always carried LEVEL — the stock horizontal, parallel to the ground, held in front of the body or resting across a forearm; a firearm has a barrel, lock and stock. Render it LARGE and clearly in the FOREGROUND with its shape and details sharp and readable — the single most eye-catching prop. Keep it firmly GRIPPED in a hand and clearly held (fingers around the handle; it may rest on a shoulder, lean on the body or touch the ground). If the weapon is a MATCHED PAIR (twin blades, dual sabers, paired daggers, a set of two), the character wields ONE IN EACH HAND, both fully drawn and held ready — the two are IDENTICAL TWINS: the SAME length, the same blade shape and width, the same guard, grip and level of detail, mirror images of each other (state this explicitly in the prompt). EXCEPTION: if the item description says the two DIFFER (different colors, different gems), keep the same size and shape but follow the stated difference faithfully — and give each variant to the exact hand the description names. Keep the weapon fully visible and in hand.
ARMOR & ACCESSORY — FAITHFUL & RECOGNIZABLE: render the armor and accessory so they are clearly RECOGNIZABLE as the SAME items shown in their IMAGES — preserve each one's silhouette, main colors, signature ornaments AND its GARMENT TYPE and material. The body ('armor') item is whatever garment its IMAGE actually shows — metal plate armor, OR a soft dress, gown, robe, coat or uniform — and you keep that MATERIAL: a soft cloth garment stays soft draping cloth and is never hardened, plated or restyled into rigid metal armor; metal plate stays metal plate. Draw them naturally in the anime style and blend them harmoniously onto the character. Keep each source item's identity intact (a faithful anime rendering of the same item). Every accessory is WORN or attached naturally in its PROPER place. Pieces with a FIXED place always go there: a HEAD piece (helmet, crown, tiara, circlet, mask, hood, headdress, decorative horns, hairpin, halo) sits ON THE HEAD; EYEWEAR (glasses, goggles, monocle, eyepatch) sits ON THE FACE over the eyes or pushed up on the forehead; a RING goes on a finger, a BRACELET on a wrist, GAUNTLETS on the hands, EARRINGS on the ears, a NECKLACE or medallion around the neck, a SCARF or stole around the neck and shoulders, EPAULETS on the shoulders, WINGS on the upper back. Only a genuinely portable piece (a fan, pouch, charm, jar, tankard, lantern, horn instrument, sash ornament) VARIES where it sits from one design to the next (held in a free hand, tucked into a belt or waist sash, or fastened as an ornament — mix up the placement of these across designs). A SHIELD is carried on one forearm with its face visible toward the viewer; when the weapon occupies BOTH hands (a matched pair wielded in each hand, or a two-handed grip), the shield is instead slung across the back with part of it showing behind the shoulders.
OUTFIT — COHERENT HEAD TO TOE: dress the whole body in ONE believable outfit whose LOWER BODY belongs with the armor in material and weight. A rigid torso piece (a plate breastplate, cuirass or torso armor) continues into matching hard-armor legs — plated greaves and thigh guards over sturdy trousers and boots — so the figure reads armored from chest to feet. Cloth attire (a robe, coat, uniform or long gown) continues in the SAME fabric down the body as a full-length robe, matching trousers or a layered skirt. A cape, cloak or mantle is an OUTER layer worn OVER a complete outfit — a full proper garment stays on the body beneath it. Always give the legs proper garments (armored legwear, trousers, or a long skirt) that match the upper body; keep skin-tight leggings or a plain bodysuit ONLY for a source item that is genuinely a form-fitting bodysuit.
...
export async function composeV3Description(input: ComposeV3Input): Promise<string> {
  ...
  // 무기 종류 앵커(2026-08-06) — wornDesc·스프라이트만으로는 종류가 소실되는 아이템이 있다
  // (날개꼴 칼→날개, 활대 큰 석궁→활 실측). 사전 값을 블록에 실어 프롬프트에 남게 한다.
  const kind = weaponKindOf(input.weaponKey);
  const content: Anthropic.MessageParam['content'] = [];
  for (const it of items) {
    const lore = it.lore ? `\nlore: ${it.lore}` : '';
    const type = it.slot === 'weapon' && kind ? `\ntype: ${kind.noun}` : '';
    content.push({ type: 'text', text: `=== ${label[it.slot]} ===${type}\nvisual: ${it.wornDesc}${lore}` });
