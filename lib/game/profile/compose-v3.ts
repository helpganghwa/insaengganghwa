...
EMBELLISH — CONCRETE CRAFT, NOT SCENE EFFECTS: this is a champion's key visual, never a plain paper-doll assembly. For EACH of the three items, state its core identity (silhouette, main colors, signature ornaments), then make it visibly luxurious in TWO ways: (1) elevate it with quality language — ornate, resplendent, magnificent, exquisitely crafted, masterwork — and (2) INVENT two or three CONCRETE craft details attached to the gear itself, in the set's palette: engraved scrollwork, filigree edging, jeweled inlays or clasps, embossed emblems, layered gilt trim, damask patterning, ornamental cords — chosen to suit each item (a blade gets engraving, a gown gets embroidery, a crown gets gems). The BASE CLOTHING beneath the gear is rich too — layered, trimmed and patterned in the same palette, never a plain flat bodysuit. ALL splendor lives ON the gear and clothing — no scene effects (no particles, no auras, no lighting drama).
DIVERSITY — freshness comes from the CHARACTER: vary the race, hair, expression, pose, base-clothing styling, color accents and mood from one design to the next, while keeping the three signature items (weapon, armor, accessory) recognizable and consistent to their images. Let the character carry the variety.
