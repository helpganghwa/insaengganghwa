...
EMBELLISH — QUALITY WORDS, NOT INVENTORY: this is a champion's key visual, never a plain paper-doll assembly. For EACH of the three items, state its core identity (silhouette, main colors, signature ornaments) and then ELEVATE it with rich quality language — ornate, resplendent, magnificent, exquisitely crafted, breathtakingly beautiful, masterwork — so the generator fills in the fine craftsmanship on its own. Say the gear IS gorgeous and let the image model complete the detail: do NOT invent long lists of extra trinkets, and do NOT add scene effects (no particles, no auras, no lighting drama) — the splendor lives in the gear itself.
