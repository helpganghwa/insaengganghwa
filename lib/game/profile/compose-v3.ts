...
//  - 미소녀(여)/미소년(남) young adult(25~29, late twenties): 동안·10대 얼굴 방지 + 과성숙 방지.
...
function subjectOf(male: boolean): string {
  return male
    ? 'a good-looking bishonen — a young man in his late twenties (about 25-29), with a refined, composed face that clearly reads as late-twenties (settled features and a defined jawline), bright expressive eyes and clean handsome features, a TALL slim long-legged 7-heads-tall body; clearly male with a flat masculine chest and a masculine hairstyle'
    : 'a beautiful bishojo — a young woman in her late twenties (about 25-29), with a refined, composed face that clearly reads as late-twenties (poised, settled features), bright expressive eyes and clean pretty features, a TALL slim long-legged 7-heads-tall body';
}
...
AGE — KEEP CONSISTENT: a young adult in their LATE twenties (about 25-29). Give a refined, composed face that clearly reads as a person in their late twenties — settled features, a defined jawline and a calm, confident look — and keep this same fresh late-twenties look consistent even with regal, heroic or ornate gear.
...
export async function composeV3Description(input: ComposeV3Input): Promise<string> {
  ...
  content.push({
    type: 'text',
    text: `Race: ${ap.race}. Hair: ${ap.hair} hair. Expression: ${ap.expression}. Pose: ${ap.pose}. Reproduce the WEAPON faithfully and in high detail from its image, rendered LARGE and prominent, firmly gripped in a hand and clearly held (if it is a matched pair, one in each hand, both fully drawn). Render the ARMOR and ACCESSORY so they stay clearly RECOGNIZABLE as the SAME items from their images — silhouette, main colors, signature ornaments — anime-styled and blended in, keeping each item's identity; vary accessory placement across designs (a fan may sit in a free hand, at the belt or as an ornament). Draw diversity from the CHARACTER (race, hair, expression, pose, base clothing, color accents, mood) while keeping the items faithful. Keep the subject a good-looking young adult in their late twenties (about 25-29) with a settled, late-twenties face, in strong Japanese anime style. Write the prompt richly detailed toward the ~1900-character budget with all three items fully described, using positive affirmative wording throughout.`,
  });
