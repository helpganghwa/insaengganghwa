 ...
 * AI 검수는 프레이밍/비율을 판정하지 않으므로(ai-review.ts는 안전+해부학 모더레이터) alpha로 선차단.
 *
...
export interface CropMetrics {
  ...
  bottomMargin: number;
  /** 바닥 밴드 평균폭 ÷ 피사체 최대폭. 클수록 넓은 단면(=발 아님). */
...
export async function detectFullBodyCrop(png: Buffer): Promise<CropResult> {
  ...
  try {
    ...
    const bottomBandRatio = bandRows > 0 ? bandSum / bandRows / maxSpan : 0;
    ...
    const wideBottom = bottomBandRatio >= 0.5; // 밴드가 최대폭의 절반 이상 = 단면(발 아님)
