...
import { GUILD_LORD_TAX_CUT, TAX_COLLECT_COOLDOWN_MIN } from './balance';
import { GuildError } from './errors';

/**
 * 영주 세금 수금 — GUILD §5.5. 그 구역 영주만, 1시간 쿨다운. 구역 누적 💎 →
 * 영주 10% + 소유 길드 풀 90%. 단일 트랜잭션, 구역 행 for update.
 */
export function collectZoneTax(input: {
  userId: string;
  zoneId: number;
}): Promise<{ lordGain: bigint; guildGain: bigint }> {
  return db.transaction(async (tx) => {
    const [z] = await tx
      .select({
        lord: zones.lordUserId,
    ...
    if (z.lord !== input.userId || !z.owner) throw new GuildError('NOT_LORD');
    if (
      z.lastAt &&
    ...
    // 영주 몫(10%, floor) / 길드 몫(잔여 = 90%+).
    const lordGain = (tax * BigInt(Math.round(GUILD_LORD_TAX_CUT * 100))) / 100n;
    const guildGain = tax - lordGain;

    await tx
      .update(profiles)
      .set({ diamond: sql`${profiles.diamond} + ${lordGain}` })
