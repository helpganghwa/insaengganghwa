...
// ── 거주 이동 (§5.5, 2026-07-29 투표 반영) ──
// 배치·집행관은 그 구역 거주자만 가능하고, 거주 이동은 인접 구역으로만·6시간 쿨타임.
// 세 규칙은 하나로 묶여 동작한다 — 쿨타임이 없으면 인접 제한이 연속 이동으로 무력화된다.
/** 거주 이동 쿨타임(분) — 6시간. 공지 투표: 쿨타임 있음 60%, 그중 6시간 최다. */
export const RESIDENCE_MOVE_COOLDOWN_MIN = 6 * 60;
/** 쿨타임 보석 단축 — 남은 1분당 💎. 게임 전반 환산(1440💎 = 하루)과 동일, 전액 360💎. */
export const RESIDENCE_SPEEDUP_GEM_PER_MIN = 1;
/** 남은 ms → 단축 비용(💎). 올림 — 1초라도 남으면 1분치를 받는다. */
export function residenceSpeedUpCost(remainMs: number): number {
  if (remainMs <= 0) return 0;
  return Math.ceil(remainMs / 60_000) * RESIDENCE_SPEEDUP_GEM_PER_MIN;
}
