...
/**
 * 길드 공지 설정/해제 — notice 권한(길드장 · 허용된 부길드장). 빈 문자열이면 공지 제거(null).
...
export async function setGuildNotice(input: { userId: string; serverId: number; notice: string }): Promise<void> {
  ...
  await db.transaction(async (tx) => {
    const m = await assertGuildPerm(tx, input.userId, input.serverId, 'notice');
    await tx
      .update(guilds)
...
/**
 * 길드 소개(공개) 설정/해제 — intro 권한(길드장 · 허용된 부길드장). 빈 문자열이면 제거(null). 목록 팝업 노출용.
...
export async function setGuildIntro(input: { userId: string; serverId: number; intro: string }): Promise<void> {
  ...
  if (containsProfanity(text)) throw new GuildError('PROFANITY');
  await db.transaction(async (tx) => {
    const m = await assertGuildPerm(tx, input.userId, input.serverId, 'intro');
    await tx
      .update(guilds)
