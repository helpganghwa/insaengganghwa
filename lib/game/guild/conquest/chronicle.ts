...
- 중립지·주인 없는 땅 점령만으로 어느 지역에 한 길드만 있게 된 경우, '하나만 남았다·몰아냈다·밀어냈다' 같은 축출 뉘앙스 금지 — 그 지역에 다른 세력이 원래 없었다(정리의 '중립지/주인 없는 땅' 표기로 판단). '유일하게 발을 들였다·홀로 세력을 넓혔다'처럼 쓴다.
...
export async function generateAndStoreChronicle(
  ...
  const capAnno = (zone: string): string => {
    ...
    return c.firstCapture ? `(중립지 첫 점령${rivalNote})` : `(주인 없는 땅 점령${rivalNote})`;
  };
  ...
  const deltaLines = [...new Set([...beforeCounts.keys(), ...afterCounts.keys()])]
    .map((g) => {
      ...
      const a = afterCounts.get(g) ?? 0;
      const lostDesc =
        neutralLost > 0 ? `${lost}곳 상실(점령전 상실 ${capLost}, 방치 중립화 ${neutralLost})` : `${lost}곳 상실`;
      const warn =
        gained > 0 && lost > 0
