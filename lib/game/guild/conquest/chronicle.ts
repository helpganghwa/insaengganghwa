...
- 축출 뉘앙스 주의: 사실표에 '중립지/무주지' 점령만 있는 지역을 두고 '하나만 남았다·몰아냈다'처럼 다른 세력이 밀려난 듯 쓴 문장은 오류다 — 원래 다른 세력이 없던 곳('유일하게 발을 들였다' 류로 고쳐라).
- 방치 중립화 주의: 사실표 '방치로 중립화된 구역'은 전투 없이 소유 길드가 관리를 안 해 주인을 잃은 것이다 — 다른 길드가 '빼앗다·점령·함락시켰다'로 쓰지 말고(공격자·교전 지어내기 금지), 그 길드가 '방치해 잃었다·관리하지 않아 중립이 되었다'로만 쓴다.
...
export async function generateAndStoreChronicle(
  ...
  const deltaLines = [...new Set([...beforeCounts.keys(), ...afterCounts.keys()])]
    .map((g) => {
      ...
      const a = afterCounts.get(g) ?? 0;
      const lostDesc =
        neutralLost > 0 ? `${lost}곳 상실(전투 ${capLost} + 방치 중립화 ${neutralLost})` : `${lost}곳 상실`;
      const warn =
        gained > 0 && lost > 0
          ? ` — 보유 수를 요약할 땐 획득과 상실을 함께 쓸 것('${gained}곳을 더해 ${a}곳' 식은 산수가 안 맞아 금지)`
          : '';
      return `· 길드 「${g}」: 전투 전 ${b}곳 → 전투 후 ${a}곳 (${gained}곳 획득, ${lostDesc}${warn})`;
    })
    .filter((s): s is string => s !== null)
    .join('\n');
  ...
  if (summary.neutralized.length > 0)
    digestSections.push(
      `■ 방치로 중립화된 구역(아무도 공격·수비하지 않아 주인을 잃음 — 전투로 빼앗긴 것이 아님):\n` +
        summary.neutralized
