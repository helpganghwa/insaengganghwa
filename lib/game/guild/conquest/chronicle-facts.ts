 ...
 *  4. 재획득 — '되찾다·탈환·도로 가져가다'는 최근(7일 안) 그 구역을 쥐었던 길드가 오늘 그 구역을 노린 경우에만.
 ...
 *  24~27(2026-09-24~26) — 줄표 · 경합 상대는 그 구역을 공격한 길드 · 병력 없던 주인과 '맞붙었다' · 보유 기간 언급 과다.
 *
...
const DEBUT = /대륙에 이름을 알|첫 등장|처음으로 (?:구역|땅|영토|깃발)(?![을를]?\s?(?:내주|내준|잃|빼앗기|넘겨|넘어))|첫 (?:영토|깃발|구역을)|새로운 이름(?!으로)|새 이름(?!으로)|새 얼굴/;
/** 21 — 지형 형세 표현. */
// '고립'은 영토를 두고 쓸 때만 — '집행관은 고립된 채' 같은 인물 묘사는 형세가 아니다(09-26).
const TOPO = /조각|비지|별도의? 거점|떨어진 (?:곳|땅|거점|영토)|고립된? (?:(?:두|세|네|\d+)\s?)?(?:곳|영토|땅|거점|구역|섬)|고립되었|고립무원|섬처럼 고립/;
/** 22 — 지역 석권 표현. */
...
export function headlineIssues(headline: string, ctx: FactCheckContext): string[] {
  ...
  if (RECAPTURE.test(plain)) {
    ...
    if (zones.length > 0 ? zones.some((z) => !recapture.has(z)) : recapture.size === 0) {
      issues.push(`제목의 '되찾다·탈환'은 최근(7일 안) 잃은 구역을 오늘 되찾은 경우에만 쓴다: 「${h}」`);
    }
  }
