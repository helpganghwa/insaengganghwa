...
import { assertLeaderOfZoneOwner } from './executor';

/**
 * 점령지 포기 — GUILD §5.4. 소유 길드 길드장이 보유 구역을 자발적으로 중립화.
...
export async function abandonZone(input: { actorUserId: string; zoneId: number }): Promise<void> {
  ...
  await db.transaction(async (tx) => {
    const { guildId, serverId, zoneName } = await assertLeaderOfZoneOwner(tx, input.actorUserId, input.zoneId);
