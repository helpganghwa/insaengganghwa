...
import { meleePointsCaseSql, meleeDecayedPointsSumSql } from '@/lib/game/melee/points';
...
export type LifetimeStats = {
  ...
  /** 대난투 누적 포인트(감쇠 없는 평생 합) — 이력서 표시용. */
  meleePoints: number;
  /** 대난투 랭킹 포인트(반감기 14일 감쇠) — 리더보드 'melee'와 동일 산식. */
  meleeRankingPoints: number;
  raidSummons: number;
  raidAttacks: number;
  raidRewards: number;
  checkinDays: number;
...
export async function getLifetimeStats(userId: string, serverId: number): Promise<LifetimeStats> {
  const [r] = (await db.execute(sql`
    ...
    melee as (
      select count(*)::int joined,
             ...
             ${sql.raw(meleeDecayedPointsSumSql('mp.final_rank', 'pc.n', 'mb.battle_date'))} ranking_points
  ...
  return {
    ...
    meleeRankingPoints: num(e.melee_ranking_points),
