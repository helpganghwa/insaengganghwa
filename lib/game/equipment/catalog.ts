...
const WEAPONS: CatalogItem[] = [
  ...
  },
  {
    ...
    region: '일반',
    tone: '위트',
    lore: '가게를 넘길 때 함께 적힌 인계 목록에는 푸줏간 한 칸, 도마 둘, 갈고리 여섯, 그리고 뼈 끊던 넓적칼 하나가 들어 있었다. 비고에는 그 넓적칼이 전 주인이 외상값 받으러 나간 뒤 안 돌아와 가게에 딸려 그대로 넘어간 것이라 적혔다. 새 주인은 그 칼을 무기랍시고 떠벌리는 손님들에게 몇 번 빌려줬다가, 빌려 간 자들이 하나같이 김샌 얼굴로 돌려주는 걸 보고 그만뒀다. 칼은 도마 옆 같은 자리에 그대로 걸려, 다음 인계 목록에도 같은 한 줄로 또 적힐 참이다.',
  ...
  },
  {
    ...
    region: '오크 부락',
    tone: '일상',
    lore: '부락 어귀에 걸린 하루 알림판에는 이렇게 적혀 있다. 첫닭에 곤봉 한 번이면 교대, 해 오를 때 두 번이면 천막 점검, 한낮엔 알림 없음, 해 질 때 한 번이면 다시 교대, 밤엔 곤봉 자루로 헐거운 말뚝 박기. 알림판 맨 밑에는 작게, 곤봉 부는 것보다 박는 게 먼저이니 헷갈리면 박으라고 덧대어 있다. 신호병이 셋 바뀌는 동안 알림판은 같은 자리에 같은 글로 걸려 있었고, 곤봉도 그 옆 못에 같은 자리로 걸렸다. 부락의 하루는 그 판 그대로 굴러간다.',
  ...
  },
  {
    ...
    region: '서쪽 화산',
    tone: '수수께끼',
    lore: '주물간에 남은 거푸집 도면 한 장에 칼 모양 골이 그려져 있고 치수와 쇳물 양이 또박또박 적혀 있다. 그런데 도면 한쪽 귀퉁이에 글씨가 다른 물음이 적혀 있다. 이 골은 누가 팠는가, 용암이 식으며 저절로 패였다는데 하필 칼 모양으로. 그 아래 답은 없고 줄만 길게 그어져 있다. 이 칼을 쥔 사람들은 하나같이 비슷한 말을 한다. 내가 휘두른 게 아니라 칼이 안쪽으로 뭔가를 끌어당기는 것 같았다고. 도면은 지금 그 주물간 못에 핀으로 박혀, 칼보다 더 자주 들여다보는 물건이 되었다.',
  ...
  },
  {
    ...
    region: '일반',
    tone: '일상',
    lore: '세금 장부 맨 끝 결산 면, 숫자가 줄줄이 맞아떨어진 아래에 다른 글씨로 한 줄이 있다. 봉 한 자루, 값 없음, 적을 칸이 없어 여기 적는다고. 서기가 평생 짚고 다닌 것이고, 이 봉을 장부 위에 탁 올린 해에는 셈이 한 푼도 안 틀렸다는 말이 있으나 장부에 근거는 없다고. 결산 도장은 그 줄을 비켜 찍혀 있다. 서기가 일을 놓은 뒤 장부는 새것으로 바뀌었지만, 누가 새 장부 같은 자리에도 같은 줄을 베껴 적어 두었다. 봉은 관아 벽 그 자리에 그대로 기대어 있다.',
  ...
  },
  {
    ...
    region: '일반',
    tone: '일상',
    lore: '갱 입구에 안전 수칙이 붙어 있다. 혼자 깊이 들지 말 것, 받침목 흔들리면 즉시 나올 것, 갱이 무너지거든 곡괭이로 벽을 빠르게가 아니라 천천히 같은 박자로 칠 것, 바깥은 그 박자를 듣고 판다는 것. 셋째 수칙 옆에 누가 나중에 잔글씨를 덧붙였다. 실제로 그렇게 사흘을 친 사람이 살아 나왔고 그 곡괭이가 이것이니, 셋째 수칙은 농담이 아니라고. 곡괭이는 지금 입구 수칙판 옆 못에 걸려 있다. 새로 든 광부들은 들어가기 전 그 잔글씨를 한 번씩 읽고 들어간다.',
  ...
  },
  {
    ...
    region: '늪지대',
    tone: '위트',
    lore: '늪 마을 아이들 놀이 규칙은 단순하다. 해 질 녘까지 개구리를 가장 많이 찌른 아이가 다음 날 대장, 세 갈래 작살은 한 사람당 하나, 못 잡아도 끝까지 던질 것. 규칙 아래 글씨가 다른 한 줄이 있다. 어떤 아이가 쥔 작살은 한 마리도 못 잡아도 그 아이가 대장이라고, 왜인지는 규칙에 안 적는다고, 적으면 놀이가 끝난다고. 규칙판은 늪가 큰 돌에 분필로 쓰여 비 오면 지워졌다가, 아이들이 다음 날 또 똑같이 베껴 썼다. 작살은 그 돌 옆에 늘 기대어 있었다.',
  ...
  },
  {
    ...
    region: '일반',
    tone: '위트',
    lore: '장터에 공연 전단이 붙었다. 오늘 밤 떠돌이 광대 한 마당, 우스운 걸음과 우스운 곤봉과 배꼽 빠지는 재담, 입장은 보고 싶은 만큼. 전단 아래 깨알 글씨로, 곤봉은 소품이며 가시는 진짜지만 위험하지 않다고 단장이 보증하고, 공연 동안 동네 시비가 줄었다는 말은 우연이라고 적혀 있다. 광대가 떠난 마을마다 며칠은 술집이 조용했다는 소문이 늘 같은 전단을 따라다녔다. 곤봉은 다음 마을 전단에 또 소품이라 적혔고, 그 가시 끝은 그만큼 더 반들반들 닳아 있었다.',
