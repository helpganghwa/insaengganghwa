...
const WEAPONS: CatalogItem[] = [
  ...
  },
  {
    ...
    region: '서쪽 화산',
    tone: '장엄',
    lore: '병참 검수 기록에 대태도 한 자루가 입고됐다고 적혀 있다. 길이가 사람 키를 넘어 보관대에 안 들어가고, 무게는 한 사람이 숨 한 번에 한 번 휘두름이 한계이며, 표준 분류 칸을 잡지 못했다. 검수관은 비고에, 화산 갈라진 틈에 며칠 누여 식혔다 하고 잿빛 물결무늬는 그때 잡힌 것이라 일부러 못 낸다고 적었다. 결국 표준 무기 목록이 아니라 별도 한 줄로 올렸다. 보관대는 새로 짜야 했고, 그 보관대에는 지금도 이 한 자루만 비스듬히 누워 있다.',
  ...
  },
  {
    ...
    region: '고대 룬 산맥',
    tone: '수수께끼',
    lore: '한 사람이 큰망치 머리에서 새는 낮은 소리를 받아 적으려 한 종이가 남아 있다. 첫 음은 길게, 다음은 같은 높이로 더 길게, 그다음은 모르겠다고. 음표를 모르는 사람이라 글로 적었는데, 줄마다 아까보다 낮게라 썼다 아니 같게라 고친 자국이 어지럽다. 끝에는 따로 들은 두 사람이 흥얼댄 게 똑같았으니 그쪽이 더 정확할 듯하여 받아 적기를 그만둔다고 적혀 있다. 종이는 망치에 끈으로 묶여 함께 전해진다. 새로 받은 사람도 한 번 귀를 대 보고는, 대개 그 종이를 펴 보다 만다.',
  ...
  },
  {
    ...
    region: '오크 부락',
    tone: '영웅담',
    lore: '부락 무공 명부에 한 줄이 있다. 당시 가장 어린 전사가, 어귀가 뚫린 날 다른 무기를 다 잃고 잡동사니 도리깨 하나로 무너진 틈을 메우고 끝까지 버텼다고. 그날 등 뒤에 섰던 일곱이 전원 같은 증언을 했다고. 명부에는 보통 무기 이름을 안 적는데 이 줄만 무기까지 적혀 있다. 그 도리깨가 부락에서 더는 웃음거리가 아니게 된 까닭을 명부가 한 줄로 못 박아 둔 셈이다. 도리깨는 명부가 걸린 천막 기둥에 함께 매여, 새 전사가 그 줄을 읽을 때 늘 눈높이에 있다.',
  ...
  },
  {
    ...
    region: '일반',
    tone: '일상',
    lore: '목축 일지에는 봄에 양 마흔, 여름에 마흔에 늑대가 자주 어슬렁댔고, 가을에도 마흔에 한 마리도 안 잃었으며, 겨울에도 마흔이라 적혀 있다. 한 해 내내 그 숫자가 마흔에서 한 번도 안 줄었다. 일지 끝에 짧게, 짐승을 맞힌 날은 손에 꼽고 무릿매 돌이 휙 지나가는 소리에 무리가 비탈 너머로 흩어진 날이 훨씬 많았다고 적혀 있다. 다음 해 일지는 다른 손이 적었는데 첫 줄이 전 목동이 쓰던 무릿매를 물려받았다는 것이다. 그해 양 수도 줄지 않았고, 무릿매는 같은 못에 그대로 걸려 있다.',
  ...
  },
  {
    ...
    region: '서쪽 화산',
    tone: '영웅담',
    lore: '생환 보고서에는 고룡 숨결로 강이 끓던 자리에 창병 줄 하나를 배치해 줄을 유지하라는 명령이 떨어졌고, 줄은 끝까지 안 무너졌으며 생환은 한 명이라 적혀 있다. 그 한 명의 무기란에 이 언월도가 적혀 있고, 비고에는 날에 비늘 긁힌 골 셋, 자루에 손 눌어붙어 그을린 자국이라 덧대어 있다. 그는 보고 외에 그날 일을 따로 진술하지 않았고, 대신 이 언월도를 늘 정성껏 손질했다. 보고서와 언월도는 같은 상자에 보관되어, 둘 다 그날을 짧게만 말한다.',
  ...
  },
  {
    ...
    region: '일반',
    tone: '위트',
    lore: '곡예단 공연 진행 순서표에는 광대가 뒤뚱걸음으로 입장하고, 접시 다섯을 돌리고, 둥근 고리날을 가장자리가 안 보이게 빠르게 돌리고, 박수 받아 인사한 뒤, 고리 개수를 확인해 정리한다고 적혀 있다. 다섯째 칸 옆에는 매번 다른 글씨로 같은 말이 적힌다. 또 하나 모자람. 그날 묵은 여관 뒷마당이 유난히 조용했다는 말이 순서표를 따라다녔지만, 단장은 늘 셋째 줄 빠르게라는 데에 밑줄만 더 진하게 그었다. 순서표는 다음 마을 천막 기둥에 또 붙었고, 다섯째 칸 옆은 또 채워질 자리로 비어 있다.',
  ...
  },
  {
    ...
    region: '일반',
    tone: '영웅담',
    lore: '서약서 원문이 남아 있다. 우리 열둘은 어깨를 맞대 한 줄을 이루며, 줄이 무너지지 않는 한 이 할버드를 쓰러뜨리지 않고, 이 할버드가 선 한 줄도 물러서지 않으며, 둘은 서로에게 기댄다는 글이다. 그 아래로 서명 열둘이 거칠게 눌러쓰여 있고, 그중 셋은 글씨가 더 흐리다. 줄이 풀린 뒤 살아남은 자가 떠난 이들 몫까지 대신 그은 것이다. 할버드는 그 서약서와 함께 한 상자에 들어갔다. 새로 줄을 세우는 자들은 제 서명을 보태기 전, 그 흐린 셋부터 손끝으로 짚어 본다.',
  ...
  },
  {
    ...
    region: '일반',
    tone: '담백',
    lore: '한 집 겨울 살림 적바림에는 장작은 뒤뜰서 직접 패고, 도끼 갈기는 동전 안 들이고 숫돌로 직접 하며, 자루는 세 번째 물푸레로 갈아 끼웠고, 아궁이는 한 번도 안 꺼졌다고 적혀 있다. 그 아래 다음 해 글씨로 한 줄, 막내가 도끼를 물려받고 아비가 날을 한 번 더 갈아 줬다고. 적바림은 해마다 같은 항목이 반복되는데 액수 칸은 거의 다 없음이다. 도끼가 한 일이 적힌 데는 없다. 다만 그 아궁이 안 꺼짐 한 줄만 해마다 빠지지 않고 다시 적혔다.',
