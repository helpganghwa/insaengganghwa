...
const ACCESSORIES: CatalogItem[] = [
  ...
  },
  {
    ...
    region: '늪지대',
    tone: '위트',
    lore: '늪 마을 한 노인이 심심풀이로 깎은, 불면 영락없이 개구리 우는 소리가 나는 작은 호각이다. 노인은 이걸로 진짜 개구리를 불러 모은다고 큰소리쳤다. 마을은 둘로 갈렸다. 한쪽은 우연이라 콧방귀를 뀌었고, 한쪽은 노인이 호각을 불면 늪 어귀가 정말 한동안 시끄러워졌다며 편을 들었다. 입씨름은 호각 한 번이면 늘 다시 불붙었다. 노인은 그 다툼을 무척 즐겨, 사람들이 모이면 굳이 한 번 더 불었다. 노인이 떠난 뒤 호각을 물려받은 손주가 시험 삼아 불자, 늪 어귀가 또 한동안 시끄러웠고 마을은 또 둘로 갈렸다.',
