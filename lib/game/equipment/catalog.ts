...
const WEAPONS: CatalogItem[] = [
  {
    ...
    region: '늪지대',
    tone: '담백',
    lore: '수선 의뢰서에는 늪 우물 바닥에서 건진 한손검 한 자루를 맡기니 쓸 만하게 갈아 달라, 값은 부르는 대로 주겠다고만 적혀 있었다. 대장장이는 그 검을 도로 돌려보내며 짧은 답을 붙였다. 갈 게 없어 그냥 보낸다고. 녹을 벗겨 보니 이 나간 데가 한 군데도 없고, 그 녹이 외려 날을 단단히 붙들고 있어 손대면 그게 풀린다고. 값은 안 받으니 그냥 차고 다니라고. 의뢰인은 정말 그 말대로 그대로 차고 다녔다. 같은 묶음의 다음 장은 다른 검, 또 다른 검으로 이어지는데, 이 검 건만 그 짧은 답 한 줄로 끝나 있다.',
  ...
  },
  {
    ...
    region: '고대 룬 산맥',
    tone: '장엄',
    lore: '산이 무너진 비탈, 큰 돌 하나에 글이 새겨져 있다. 여기, 산의 화 한 줌이 떨어진 자리, 그것으로 이 망치를 벼렸다고. 그 아래로 세월 따라 다른 손들이 한 줄씩 더 새겼다. 들어 보았다, 두 발이 땅에 박히는 줄 알았다고 한 줄. 내려친 자리에 한참 낮은 울림이 남더라고 그 밑에. 맨 아래에는 글씨 서툰 한 줄이 있다. 나는 못 들었다, 너무 무거워 들지를 못해서. 망치는 지금 그 돌 곁에 세워져 있다. 지나는 사람마다 한 번 들었다 도로 내려놓고는, 제 줄을 새길지 말지 한참 서 있다 그냥 간다.',
  ...
  },
  {
    ...
    region: '오크 부락',
    tone: '위트',
    lore: '장터 떨이 스물셋 번은 오크 부락에서 나온 외날 도끼 하나였다. 자루에 큰 어금니가 박혀 있고 족장 본인 것이라 했으며, 잘 든다는 보증이 붙었다. 값을 부르기 시작했는데 아무도 안 불렀다. 다시 불러도 잠잠했다. 파는 사람이 어금니 사연을 곁들였다. "이빨이 둘이면 무는 힘도 둘이라 했답니다." 듣던 누가 받았다. "그게 무슨 소리요?" 파는 사람도 답을 못 했다. 그날도 그다음 날도 끝내 안 팔렸다. 도끼는 창고로 들어갔다가 같은 스물셋 번으로 또 목록에 올랐고, 잘 든다는 보증만 매번 그대로 따라붙었다.',
  ...
  },
  {
    ...
    region: '일반',
    tone: '일상',
    lore: '칼과 함께 오는 종이에는 주의 넷만 적혀 있다. 첫째, 이 검은 천 자루 중 하나이니 네 것이라 여기지 마라. 둘째, 이가 나가면 갈고 자루가 닳으면 그 자리에서 새로 감아라. 셋째, 부러져도 슬퍼하지 마라, 다음 자루를 꺼내면 된다. 넷째, 이름을 붙이지 마라, 붙이면 잃었을 때 곤란해진다. 그 종이에는 쓰는 법도, 내력도 없다. 주의 넷이 전부다. 그 넷을 그대로 지킨 병사들이 이 검으로 변경을 여러 번 막아냈고, 그래서 종이에 더 보탤 줄이 끝내 생기지 않았다.',
  ...
  },
  {
    ...
    region: '일반',
    tone: '수수께끼',
    lore: '길잡이가 마을 사이 거리를 적어 둔 표가 있다. 보리골에서 갈대마을이 이틀, 갈대마을에서 잿재울이 사흘, 잿재울에서 옹기점이 하루 반. 그 줄들 사이에 글씨가 다른 한 칸이 끼어 있다. 떡갈나무 지팡이 짚은 노인과 함께 가면 거리는 같으나 그해 그 길가 마을들 농사가 잘된다고, 근거는 없고 지우려다 그냥 둔다고. 표를 베끼는 사람마다 그 칸을 빼려다 끝내 같이 옮겨 적었다. 노인이 지금 어느 마을에 있는지는 표에 없고, 지팡이만 한 헛간에 세워진 채 다음 베끼는 손을 기다린다.',
  ...
  },
  {
    ...
    region: '일반',
    tone: '영웅담',
    lore: '아이들이 돌 던지며 세는 동요가 있다. 하나에 눈밭에 엎드린 사냥꾼, 둘에 외양간 노린 늑대, 셋에 사흘 밤, 넷에 활은 하나. 다섯에 늑대 머리 떨어지고, 여섯에 양은 다 살고, 일곱에 영웅이라 부르니, 여덟에 그는 그냥 웃네, 아홉에 운이 좋았다고, 열에 추웠을 뿐이라고. 열까지 세면 술래가 바뀐다. 정말 그 겨울 한 사냥꾼이 사흘을 엎드려 우두머리를 잡았고, 활은 지금도 그 집 벽에 걸려 있다. 동요의 열을 셀 때 아이들 목소리가 늘 한 번 작아지는데, 누가 그러라 한 적은 없고 그냥 대대로 그렇게 됐다.',
  ...
  },
  {
    ...
    region: '늪지대',
    tone: '일상',
    lore: '장물 사고파는 자의 장부에 늪 갈대창 한 자루가 올라 있었다. 값은 거의 없다고, 집집마다 문가에 한 자루씩 있는 물건이라고 적혔다. 비고에는 훔친 놈도 무기인 줄 알고 가져왔다가 자루에 밴 빨랫줄 받친 흔적을 보고 김이 샜다고 덧대어 있었다. 되팔려 내놨지만 아무도 안 샀다. 늪 사람이면 제 문가에서 하나 들면 그만이니까. 결국 그자도 그 창을 제 가게 문가에 세워 두고 늘어진 빨랫줄을 받쳤다. 훔쳐져 한 바퀴 돌아온 갈대창은 그렇게 또, 무기가 아니라 살림으로 제자리를 찾았다.',
  ...
  },
  {
    ...
    region: '서쪽 화산',
    tone: '장엄',
    lore: '이 대검 이야기는 마을을 건너며 한 대목씩 어긋난다. 화산 마을에선 가장 깊은 화덕에서 벼려 식는 데 보름이 걸렸다고 한다. 다음 마을에선 화산 틈에 며칠 누여 식혔다가 되고, 그다음 마을에선 용이 직접 불어 만들었다가 되며, 더 멀리 가면 휘두르면 산이 한 번 운다는 말까지 붙는다. 정작 그 칼을 만진 늙은 대장장이는 보름도 며칠도 다 틀렸다며, 다만 너무 무거워 숨 한 번에 한 번밖에 못 휘둘렀던 것만은 맞다고 했다. 칼은 지금 그 대장간 벽에 걸려 있고, 이야기는 칼보다 훨씬 먼 마을까지 가 있다.',
  ...
  },
  {
    ...
    region: '오크 부락',
    tone: '위트',
    lore: '부락 마당에서 열린 다툼 판결을 옮긴다. 갑이 을의 천막을 둔기로 부쉈다는 사건이었다. 을은 갑이 휘둘렀다 하고, 갑은 들고만 있었는데 을이 알아서 자빠졌다 했다. 증거물은 뼈를 통째로 묶어 만든 둔기 하나, 흠은 없었다. 판결은 이러했다. 둔기가 멀쩡하니 휘둘렀다 단정 못 한다. 다만 갑이, 그 둔기 만든 자가 같은 날 비탈에서 굴러 죽은 일을 두고 그게 제 솜씨 증명이라 우겨 마당을 어지럽힌 죄로, 을의 천막을 다시 세워 줄 것. 둔기는 보관 창고에 들어갔다가, 다음 다툼의 증거물로 또 불려 나왔다.',
  ...
  },
  {
    ...
    region: '일반',
    tone: '일상',
    lore: '다리 통행세 영수증 묶음에는 문지기가 여백에 적어 둔 잔글씨가 영수증 숫자보다 더 많이 섞여 있다. 한 장 옆에는 소 두 마리 끌고 온 자가 시끄러웠으나 셈은 안 속였다고. 다른 장에는 급한 전령이라 그냥 보냈고 창은 안 들었다고. 또 다른 장에는 오늘 비가 와 창에 앉은 먼지가 빗물에 줄줄이 흘러내렸다고. 문지기가 떠난 뒤 다음 사람이 그 묶음을 물려받았는데, 며칠 안 가 그도 여백에 제 잔글씨를 적기 시작했다. 창은 여전히 초소 벽 같은 자리에, 같은 각도로 기대어 있다.',
