...
const WEAPONS: CatalogItem[] = [
  ...
  },
  {
    ...
    region: '일반',
    tone: '영웅담',
    lore: '아이들이 신나게 손뼉 치며 부르는 동요가 있다. 하나에 눈밭에 엎드린 사냥꾼, 둘에 외양간 노린 늑대, 셋에 사흘 밤, 넷에 활은 하나, 다섯에 늑대 머리 떨어지고, 여섯에 양은 다 살고, 일곱에 영웅이라 부르니, 여덟에 그는 그냥 웃네, 아홉에 운이 좋았다고, 열에 추웠을 뿐이라고. 정말 그 겨울 한 사냥꾼이 사흘을 엎드려 우두머리를 잡았고, 양 한 마리 안 잃었다. 활은 지금도 그 집 벽에 자랑처럼 걸려 있다. 동요의 열을 외칠 때 아이들은 누가 시키지도 않았는데 가장 크게 소리 지르고, 그러고는 까르르 웃으며 술래를 바꾼다.',
  ...
  },
  {
    ...
    region: '일반',
    tone: '일상',
    lore: '밤 골목 점검표에는 초경에 동쪽 골목과 우물가와 빈집 문을 돌아 이상 없음, 이경에 같음에 개가 짖었으나 별일 아님, 삼경에 같음에 창은 어귀에 세워 둠이라 적혀 있다. 칸마다 표시뿐, 사건 적힌 줄은 거의 없다. 점검표 맨 밑에 한 줄, 창이 어귀에 서 있는 한 이 골목은 문단속을 두 번 안 한다고. 야경꾼이 바뀌어도 표 양식은 그대로고, 새 야경꾼도 첫날부터 같은 칸을 같은 표시로 채웠다. 창은 표가 걸린 초소 그 자리, 같은 각도로 기대어 있다.',
  ...
  },
  {
    ...
    region: '일반',
    tone: '영웅담',
    lore: '전령 전달 기록부에는 발신이 본진, 전할 내용은 봉인, 전령은 깃대 듦, 도착은 미도착이라 적혀 있다. 그 아래 다른 손이 비고를 채웠다. 깃발이 쓰러지면 줄도 무너지기에 전령은 소식 대신 그 자리를 택했고, 깃대 끝에 도끼날을 박았으며, 그날 깃천은 누더기가 되었으나 깃대는 한 번도 안 기울었다고. 전할 내용 칸은 봉인된 채 끝내 안 열렸다. 기록부는 그 미도착 한 줄에서 멈추고, 폴액스는 깃대째 본진 마당에 세워져 새 전령들이 지나며 한 번씩 올려다본다.',
  ...
  },
  {
    ...
    region: '일반',
    tone: '담백',
    lore: '불난 뒤 마을 복구 분담표에는 탄 그루터기와 줄기 정리를 거둠낫 가진 사람 누구나 구역을 나눠 한 뼘씩 하고, 도구가 모자라면 돌려쓰라 적혀 있다. 표 끝에 다음 해 글씨로 한 줄, 그 밭이 봄에 푸르게 돋았음을 확인함이라 덧대어 있다. 누가 그 낫을 휘둘렀는지는 분담표에 이름이 없다. 여럿이 번갈아 잡았기 때문이다. 거창한 사연 대신 분담 구역과 한 줄 확인만 적혀 있고, 낫은 마을 창고 그 분담표 옆 못에 걸려 다음 봄을 기다린다.',
  ...
  },
  {
    ...
    region: '일반',
    tone: '수수께끼',
    lore: '옛 노래 채집 노트에는 무쇠 부채를 펴고 접을 때 가락이 새며 늙은이들이 입을 모아 자장가라 한다고, 가사를 받아 적겠다고 적혀 있다. 그 아래 가사 칸이 줄줄이 그어졌는데 전부 비어 있고, 칸 옆 잔글씨만 채워져 있다. 귀 대고 받아 적다 졸았다고. 다시 시도했으나 또 졸았다고. 두 사람을 시켜도 둘 다 졸았다고. 마지막엔 가사를 못 받아 이 노트는 빈 채로 둔다고. 부채는 그 빈 노트와 함께 보관되고, 새로 받은 사람도 한 번 펴 보다 가락을 듣고는 노트 앞에서 꾸벅 존다.',
