...
export const CRON_MAX_GAP_MS: Record<string, number> = {
  ...
  // 15분(7/22/37/52분) — 월드 피드의 1위 교체 감지. 정지 = 유저 랭킹 5종 + 길드 전투력·점령지
  //   1위가 바뀌어도 피드에 안 뜬다(스냅샷 비교라 놓친 교체는 소급되지 않음).
  'rank-leader': 60 * MIN, // (2026-08-11 등재)
  // 매일 KST03시 — 만료/수령완료 우편 · world_events · 채팅 · 귓속말 · 길드 가입신청 정리의
  //   유일한 경로. 정지 = 이 테이블들이 전부 단조 증가한다.
  'mail-expire': 25 * HOUR, // (2026-08-11 등재)
  // 10분 — Play 환불 회수(소모 재시도·취소 확인·voided 스윕). **한 번도 동작한 적 없던 채로
  //   방치된 전력이 있다**(조회 경계 30일로 매번 400 → 예외가 크론 전체를 죽임, 2026-09-11).
