...
describe('findPastContextZoneKeys — 과거 회고 문장의 구역 마커는 리플레이 트리거에서 제외', () => {
  it('"어제 …" 문장의 마커만 건너뛰고, 같은 구역의 뒤 언급은 남긴다', () => {
    ...
    // 09-05: 회고 문장에만 나오는 구역(불탄 마을)은 건너뛰지 않고 그 자리에서 재생 — 뒤에도 나오는 잿더미 폐허만 뒤로 미룬다.
    expect(names((k) => deferred.has(k))).toEqual(['잿더미 폐허']);
    expect(names((k) => !deferred.has(k))).toEqual(['불탄 마을', '잿더미 폐허', '흑요석 보루', '재의 길목', '포자 습지']);
  });
  ...
  it('회고 문장에만 나오는 구역은 건너뛰지 않는다(그 자리에서 재생, 2026-09-05)', () => {
    const paras = ['어제 얻은 {z|용암 하구|9}를 지키던 수비수는 {g|티모집사|31}의 공세에 밀렸다. {g|왕실|25}은 {z|재의 길목|6}을 거두었다.'].map((p) => parseChronicleSegments(p));
