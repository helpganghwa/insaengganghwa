...
describe.skipIf(skip)('머니경로 — completePurchase/refundPurchase DB 통합', () => {
  ...
  afterEach(async () => {
    ...
    await testDb.execute(
      sql`delete from iap_refunds where user_id = ${TEST_USER_ID}::uuid and created_at >= ${testStart}`,
    );
    await testDb.execute(
      sql`delete from iap_orders where user_id = ${TEST_USER_ID}::uuid and portone_order_id like 'test\\_%'`,
    );
    await testDb.execute(
      sql`delete from mailbox where user_id = ${TEST_USER_ID}::uuid and title = '결제 환불 안내' and created_at >= ${testStart}`,
    );
