...
### 3.7 자랑 단위 — 장비 단위 / 장비 전체

공유 카드(동적 OG)는 두 종류: (1) **장비 단위** — 장비 1개의 외관·강화·초월·전투력 상세, (2) **장비 전체** — 무기/방어구/장신구 3슬롯 세트 + 프로필. 등급 표기 없음(등급 시스템 자체가 없음) — 강화/초월 레벨 + 전투력으로 표현.
...
- **고정 390 스케일**: viewport `width=390`(layout.tsx) + 앱 셸 고정 `w-[390px] mx-auto`(safe-area). 모든 화면을 390 컬럼 안에서 구현 — 모바일 동일 레이아웃·데스크톱 중앙 정렬. WIREFRAMES §0 참조
