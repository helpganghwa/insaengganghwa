...
### 11.7 서버 액션 후 갱신 — `router.refresh()` 중복 금지 (2026-07-23)
- **서버 액션은 mutation 후 현재 라우트(layout+page)를 서버에서 재렌더해 응답 RSC에 실어 보낸다** — 별도 네트워크 요청 없이 클라 트리가 갱신된다. 즉 액션을 `await`한 직후엔 헤더(전투력·다이아 등 layout 데이터)까지 이미 새 값이다.
- ⚠ 따라서 **액션 성공 직후 `router.refresh()`는 중복**이다 — 별도 GET RSC refetch(`/route?_rsc=…`)로 **같은 페이지를 한 번 더 렌더**해 요청·함수호출·DB쿼리가 2배가 된다(강화 수령 실측: 요청 2→1). 정상 경로에서는 호출하지 말 것. 게이지·HP 등 크리티컬 UI는 refresh를 기다리지 말고 **액션 응답에 다음 상태를 실어 즉시 반영**한다(강화 `nextJob`·레이드 `totalDamage` 패턴).
- **유지해야 하는 refresh**: 액션 응답이 실패(reject/에러)해 revalidate가 안 일어난 경로의 재동기화, 타이머/가시성 복귀(체크인 자정·`RefreshOnResume`), 액션을 거치지 않는 순수 클라 상태 변경.
- ⚠ **`revalidatePath`는 별도 리패치를 안 만들지만, 액션의 현재-페이지 재렌더가 `layout`까지 포함**해 매 액션마다 `loadLayoutData`(프로필·우편·친구요청·장비 등 병렬 쿼리)가 재실행된다. 고빈도 액션(강화·레이드)에서 누적 부하. 헤더에 실시간 반영이 꼭 필요한 값만 layout에 두고, 나머지는 클라 컨텍스트(`DiamondContext` 패턴)로 빼 layout 재렌더 시 쿼리를 피하는 것을 우선 고려.
