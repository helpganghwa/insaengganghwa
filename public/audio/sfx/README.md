...
지금은 이 **11개만** 만들면 된다(나머지 UI·보상음은 보류). 코드 트리거는 이미 연결돼 있어,
아래 파일명대로 `public/audio/sfx/`에 넣고 빌드하면 적용된다(`scripts/build-asset-versions.ts`가 만든 매니페스트에 있는 파일만 요청한다. 로컬 dev에서는 그 스크립트를 한 번 돌릴 것). 프롬프트는 영어 그대로 복붙.
...
- **이 12종은 `lib/game/sound.ts`에 전용 Web Audio 합성 보이스가 이미 디자인돼 있다**(오실레이터+
  필터드 노이즈+엔벨로프). 그래서 **AI 샘플 없이도 바로 소리가 난다** — 파일은 선택(덮어쓰기용).
- 합성음이 어색하면 코드 수치(주파수·길이·노이즈 필터)를 조정하면 된다(파일 생성 불필요).
- 더 사실적인 음을 원하면 위 프롬프트로 샘플을 만들어 `public/audio/sfx/<name>.webm`에 넣으면 그 효과만 교체.
