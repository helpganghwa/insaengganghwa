...
> **현재 기획: 전 화면 공통 BGM(단일 메인 테마).** 모든 화면이 `hub.m4a` 하나를 공유하며,
> 화면 전환 시 재시작 없이 끊김 없이 이어 재생된다(`lib/audio/bgm-map.ts`). idle 게임 특성상
> 한 테마를 길게 트는 게 자연스럽고, 1인 개발 ROI도 좋다. **지금 만들 파일은 `hub.m4a` 하나뿐.**
>
> 아래 화면별 트랙/프롬프트 표는 **향후 화면별 분리를 도입할 때**의 참고용이다(인프라는 이미 지원 —
> bgm-map에 경로별 분기만 추가하면 됨). 지금은 hub만 채우면 된다.

배경음악(모던 시네마틱). **Suno에서 생성(Instrumental ON)** 후 아래 파일명으로
