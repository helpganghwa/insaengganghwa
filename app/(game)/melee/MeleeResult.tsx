...
const KILLED_MSGS: ((a: string, t: string, d: string) => string)[] = [
  ...
  (a, t, d) => `${josa(`${a}#{이}`)} 결정타를 꽂는다. ${d} — ${t}, 아레나에서 탈락.`,
...
const SURVIVE_MSGS: ((a: string, t: string, d: string, hp: string) => string)[] = [
  ...
  (a, t, d, hp) => `${a}의 공격이 ${josa(`${t}#{을}`)} 강타! ${d} 피해 — ${t}, 체력 ${hp}로 버틴다.`,
  (a, t, d, hp) => `${t}, ${a}의 ${d} 공격을 이 악물고 견딘다. (체력 ${hp})`,
  (a, t, d, hp) => `${josa(`${a}#{이}`)} ${d}의 일격을 날린다. ${t}, 체력 ${hp}로 반격을 노린다.`,
  (a, t, d, hp) => `격렬한 공방! ${a}의 ${d} 피해에도 ${josa(`${t}#{은}`)} 체력 ${hp}로 살아남는다.`,
