...
const SURVIVE_MSGS: ((a: string, t: string, d: string, hp: string) => string)[] = [
  (a, t, d, hp) => `${a}의 공격이 ${josa(`${t}#{을}`)} 강타! ${t}, 체력 ${josa(`${hp}#{으로}`)} 버틴다. (-${d})`,
  (a, t, d, hp) => `${josa(`${a}#{이}`)} ${d}의 일격을 날린다. ${t}, 체력 ${josa(`${hp}#{으로}`)} 진영을 지킨다.`,
  (a, t, d, hp) => `격전! ${a}의 ${d} 피해에도 ${josa(`${t}#{은}`)} 체력 ${josa(`${hp}#{으로}`)} 견딘다.`,
