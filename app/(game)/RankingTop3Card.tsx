...
/**
 * 홈 §1 — Top 3 명예의 전당 카드. 진입마다 metric(5종) 중 데이터 있는 하나를 랜덤 노출.
...
export async function RankingTop3Card() {
  const decks = (
    await Promise.all(
      METRICS.map(async (m) => ({ ...m, top: await getRankingTop(m.metric, 3) })),
    )
  ).filter((d) => d.top.length > 0);
  if (decks.length === 0) return null;

  const deck = decks[crypto.getRandomValues(new Uint32Array(1))[0]! % decks.length]!;
