...
export function GachaBoxCard({
  ...
  const pull = (n: number) => {
    ...
    openAction(slot, n)
      ...
      .catch(() => {
        // ⚠ "다시 시도"를 권하지 않는다(2026-09-12). 서버가 개봉을 커밋한 뒤 응답만 유실된
        // 경우가 섞여 있어, 재시도하면 상자를 한 번 더 쓴다(아이템은 이미 인벤에 있으므로 가치
        // 손실은 없고 '의도보다 많이 열림'이다). 멱등키를 붙이려면 개봉 로그에 스키마가 필요해
        // 별도 회차로 미뤘고, 그때까지는 **먼저 확인하라**고 말하는 것이 정확하다.
        showError('개봉 결과를 확인하지 못했어요. 인벤토리를 확인한 뒤 다시 시도해 주세요.');
        setOptimistic(null);
        setErrorTick((t) => t + 1); // 자동 반복 중지 신호
      })
