...
import { runAppleCheckout } from './apple-checkout';
...
export function ShopTabs({
    ...
    if (returnPaymentId) {
      void (async () => {
        ...
        if (v.status === 'success') {
          ...
          // already(웹훅 선지급)는 유저 관점에선 그냥 성공 — 첫 안내에 "이미 지급된 결제"라고
          // 말하면 중복 결제로 오해한다(2026-08-22 제보). 문구 분기 없이 단일 완료 안내.
          setPayNotice({
            title: '구매 완료',
            body: '지급이 완료되었습니다. 상단 다이아와 상점 화면에서 확인해 보세요.',
          });
  ...
  const onPay = (productId: string) => {
    ...
    void (async () => {
      ...
      // 플레이스토어 앱(TWA)이면 Play 결제, 앱스토어 앱이면 Apple 결제(포트원 결제창 노출 금지 — 스토어 정책), 웹은 포트원. 결과 형태 동일.
      const platform = platformClient();
      const r =
        platform === 'twa'
          ? await runPlayCheckout(productId).catch(() => ({ ok: false, reason: 'create', code: 'NETWORK' }) as const)
          : platform === 'ios'
            ? await runAppleCheckout(productId).catch(() => ({ ok: false, reason: 'create', code: 'NETWORK' }) as const)
