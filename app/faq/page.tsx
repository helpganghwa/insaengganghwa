...
const FAQS: { q: string; a: string; body?: ReactNode }[] = [
  ...
  {
    q: '앱을 설치해야 하나요?',
    a: '설치하지 않아도 웹 브라우저에서 바로 플레이할 수 있어요. Google Play에서 앱으로 설치하면 같은 계정으로 이어서 즐길 수 있고 알림도 앱으로 받습니다. 앱에서 충전한 내역은 Google Play를 통해 결제·환불됩니다.',
  },
  {
    q: '강화는 어떻게 하나요?',
  ...
  },
  {
    ...
    a: '아니요, 장비가 파괴되거나 사라지는 일은 없어요. 강화 결과는 메가(두 단계 ↑)·성공·유지·하락 네 가지이고, 단계별 확률은 확률 공시 페이지에 전부 공개되어 있습니다.',
    body: (
      <>
        아니요, 장비가 파괴되거나 사라지는 일은 없어요. 강화 결과는 메가(두 단계 ↑)·성공·유지·하락
        네 가지이고, 단계별 확률은{' '}
  ...
  {
    q: '전투력은 어떻게 계산되나요?',
    a: '착용 여부와 상관없이, 보유한 모든 장비의 강화·초월 수치를 합산한 값이에요. 그래서 장착 중인 3개만 키우는 것보다 인벤토리의 장비까지 골고루 강화하는 게 훨씬 유리합니다. 이 전투력이 레이드 데미지, 대난투 승률, 랭킹에 그대로 쓰여요.',
  },
  {
    q: '초월이 뭔가요?',
