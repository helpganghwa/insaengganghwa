...
export default function Doc() {
  return (
    <>
      ...
      <Warn>소환에 쓴 다이아는 돌려받지 못한다. 아무도 참여하지 않은 채 끝나도 마찬가지다.</Warn>
      ...
      <Warn>
        한 번 참가하면 스스로 나갈 수 없다. 정산될 때까지 정원 한 자리와 동시 진행 한 칸을 계속
        차지한다.
      </Warn>
      ...
      <UL>
        <LI>
          공격 횟수는 레이드마다 {fmtInt(RAID_BASE_ATTACKS)}회이며, 남은 횟수는 정산과 함께
          사라진다.
        </LI>
        <LI>
        ...
        <LI>
          레이드 중에 <DocLink slug="enhance">강화</DocLink>로 전투력을 올리면 다음 공격부터
          반영된다.
        </LI>
        <LI>
      ...
      />
      <Note>
        그 위로도 같은 폭으로 오른다. 비용은 레이드마다 따로 세므로 새 레이드에서는 다시 가장 낮은
        단계부터 시작한다.
      </Note>
      ...
      <UL>
        ...
        <LI>
          돌파는 참가자 전원의 누적 데미지 합으로 판정되며, 기여도 순위는 화면에만 표시된다.
        </LI>
      </UL>
      ...
      <UL>
        <LI>
          소환과 참여를 합쳐 하루 {fmtInt(RAID_DAILY_CAP)}번이며, 한국 시간 자정에 초기화된다.
          <Fn n={4} />
        </LI>
        <LI>
          동시에 진행할 수 있는 레이드는 {fmtInt(RAID_MAX_CONCURRENT_PER_USER)}개이며, 직접 소환한
          것과 남의 레이드에 참여한 것을 함께 센다.
          <Fn n={5} />
        </LI>
      </UL>
      ...
      <FnList
        notes={[
          ...
          '공유 링크로 들어온 사람은 공개 설정과 상관없이 요청부터 등록된다. 지목해 보낸 초대는 바로 참여된다. 어느 쪽이든 레이드가 소환된 서버에 캐릭터가 있어야 참여할 수 있다.',
          '어느 부위 상자인지는 레이드와 페이즈로 정해진다. 상자는 그 레이드가 소환된 서버로 지급된다.',
          '참가 요청은 수락된 날에 센다.',
          '시간이 끝났어도 정산 전이면 한 칸을 차지한다. 칸이 모자랄 때는 끝난 레이드에 들어가면 그 자리에서 정산된다.',
        ]}
      />
      <P>
