...
export const meta: WikiDocMeta = {
  ...
  cat: '경쟁',
  title: '레이드',
  summary: '난이도를 골라 소환하는 보스, 돌파한 페이즈와 달성 보상만큼 상자.',
  sections: [
    { id: 'open', label: '소환' },
    { id: 'tier', label: '난이도' },
    { id: 'join', label: '참여' },
...
export default function Doc() {
  return (
    <>
      ...
      <UL>
        <LI>
          보스와 난이도를 선택해 소환하며, 소환한 사람이 방장이자 첫 참가자가 된다. 하루 첫 소환은
          무료이고 두 번째부터는 {fmtInt(RAID_TIERS.easy.openCost)} 다이아가 든다(난이도와 무관하게 같은
          비용).
          <Fn n={1} />
        </LI>
        <LI>
      ...
      <UL>
        ...
        <LI>수락: 참가 요청이 등록되고, 방장이 수락해야 참여된다.</LI>
              <LI>소환한 사람이 공유하는 링크로 들어오면 비공개 레이드여도 수락 없이 바로 참여한다. 참여자가 퍼뜨린 링크로 들어오면 참가 요청 뒤 소환자의 수락이 필요하다.</LI>
      </UL>
      ...
      <UL>
        ...
        <LI>
          누적 돌파 페이즈가 난이도 표의 달성 페이즈에 이르면 달성 보상 상자를 추가로 받는다.
        </LI>
