...
export default function Doc() {
  return (
    <>
      <H2 id="open">소환</H2>
      <P>
        보스를 하나 고르고 {fmtInt(RAID_OPEN_COST_DIAMOND)} 다이아를 내면 레이드가 열린다. 소환한
        사람이 방장이자 첫 참가자가 되고, 뒤에 들어오는 사람은 아무것도 내지 않는다.
      </P>
      <P>
      ...
      <Warn>소환에 쓴 다이아는 돌려받지 못한다. 아무도 들어오지 않거나 페이즈를 하나도 돌파하지 못한 채 끝나도 마찬가지다.</Warn>

      <H2 id="join">참여</H2>
      <P>
        정원은 방장까지 {fmtInt(RAID_MAX_PARTICIPANTS)}명이다. 누구에게 보일지는 열 때 친구 공개와
        길드원 공개를 따로 정하고, 나중에 바꾸지 못한다. 각각 세 가지다.
      </P>
      <UL>
        <LI>비공개: 참여 가능한 레이드 목록에 뜨지 않는다. 링크나 초대로만 들어온다.</LI>
      ...
      <P>
        데미지는 공격하는 순간의 총 전투력에서 나온다. 빗나가는 일은 없고, 같은 전투력이라도 한 대마다
        폭이 오르내린다. {bpPct(RAID_CRIT_RATE_BP)} 확률로 크리티컬이 터지면 그 한 대는 크게 들어간다.
      </P>
      <P>
        레이드 중에 전투력을 올리면 다음 공격부터 반영된다. 기본 횟수를 다 쓰면 다이아로 추가 공격을
        한 번씩 사고, 값은 몇 번째로 사느냐에 따라 오른다.
      </P>
      <Tbl
        head={['추가 공격', '1회 값']}
      ...
      <P>
        ...
        {fmtInt(RAID_PHASE1_HP_MAX)} 사이로 정해지고, 다음 페이즈로 갈수록 높아진다. 마지막
        페이즈는 없다.
      </P>
      <P>
        돌파는 참가자 전원의 누적 데미지 합으로 잡는다. 기여도 순위는 화면에 뜨지만 보상을 가르지
        않는다.
      </P>
      ...
      <P>
        보상은 돌파한 페이즈 수를 따라간다. 하나 돌파할 때마다 보급 상자{' '}
        {fmtInt(RAID_PHASE_DROP_BOXES)}개가 나오고, 어느 부위 상자인지는 레이드와 페이즈로 정해져
      ...
      <Warn>공격을 한 번도 하지 않은 참여자는 보상에서 빠진다. 들어가기만 하고 공격을 안 하면 하루 횟수만 줄어든다.</Warn>

      <H2 id="limit">하루 한도</H2>
      <P>
        소환과 참여를 합쳐 하루 {fmtInt(RAID_DAILY_CAP)}번, 한국 시간 자정에 다시 찬다. 요청은 수락된
        날에 세고, 거절당하거나 방치된 요청은 횟수를 쓰지 않는다.
      </P>
      <P>
        동시에 걸어둘 수 있는 레이드는 {fmtInt(RAID_MAX_CONCURRENT_PER_USER)}개다. 내가 소환한 것과
        남의 것에 들어간 것을 함께 세고, 시간이 끝났어도 정산 전이면 한 칸을 차지한다. 칸이 모자랄
        때는 끝난 레이드를 열면 그 자리에서 정산된다.
      </P>
      <P>
