...
export default function Doc() {
  return (
    <>
      <H2 id="flow">진행</H2>
      <P>
        강화에는 돈이 들지 않는다. 시작해 두면 시간이 흐르고, 다 기다렸다가 수령 버튼을 누르면
        결과가 나온다. 놔둔다고 저절로 끝나지는 않는다.
      </P>
      <P>
        확률은 기다린 시간에 비례한다. 절반만 기다리고 수령하면 공시 확률의 절반. 게이지가 다 차면
        공시 확률 그대로이고, 그 뒤로는 며칠을 놔둬도 같은 값이다. 급하면 일찍 수령해도 된다.
        확률만 낮아질 뿐 따로 손해 보는 건 없다.
      </P>
      <P>
        강화 칸은 부위당 2개, 세 부위 합쳐 6개다. 장착하지 않은 장비도 강화할 수 있다. 수령하면
        다음 단계 강화가 바로 다시 시작된다.
      </P>
      <Note>
      ...
      <Warn>하락을 막아 주는 보호권 같은 아이템은 없다. 안전 구간을 넘어서면 수령할 때마다 하락이 나올 수 있다.</Warn>
      ...
      <P>
        ...
        시간을 전부 줄이면 최대 확률로 바로 수령할 수 있다. 필요한 만큼만 차감되고, 환산 비율은
        시작 시점 값으로 고정된다.
      </P>
      <P>
