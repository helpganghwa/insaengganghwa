...
export const meta: WikiDocMeta = {
  ...
  cat: '성장',
  title: '강화',
  summary: '기다린 시간에 비례해 확률이 올라가고, 성공·유지·하락 판정을 받는다.',
  sections: [
    { id: 'flow', label: '진행' },
...
export default function Doc() {
  return (
    <>
      <H2 id="flow">진행</H2>
      <UL>
        <LI>강화는 무료이며, 강화를 등록하고 기다렸다가 강화를 시도하면 결과가 나온다.</LI>
        <LI>
          확률은 기다린 시간에 비례. 절반 기다리면 공시 확률
          <Fn n={1} />의 절반, 다 차면 공시 확률 그대로.
        </LI>
        <LI>
          일찍 강화해도 확률만 낮을 뿐 손해는 없다.
          <Fn n={2} />
        </LI>
        <LI>
        ...
        <LI>강화 시도 후 결과가 나오면 다음 단계 강화가 바로 이어진다.</LI>
      </UL>
      ...
      <UL>
        ...
        <LI>
          ...
          <Fn n={3} />으로 두 단계 상승. 표기되는 성공 확률은 대성공까지 합친 값.
        </LI>
        <LI>+{fmtInt(SAFE_MAX_LEVEL)}까지는 안전 구간. 실패해도 유지.</LI>
        <LI>
          그 위부터 단계별 하락 확률이 생긴다. 하락은 기다린 시간과 무관하게 고정.
          <Fn n={4} />
        </LI>
        <LI>그래서 안전 구간을 넘긴 뒤에는 게이지를 다 채우고 강화하는 것이 좋다.</LI>
        <LI>
          하락은 한 번에 한 단계, 그 주기의 +{fmtInt(SAFE_MAX_LEVEL)} 아래로는 하락하지 않는다.
        </LI>
      </UL>
      ...
      <UL>
        ...
        <LI>
          ...
          {fmtMs(enhanceDurationMs(CYCLE_LEN - 1))} 걸리면 다음 주기인 +
          {fmtInt(2 * CYCLE_LEN - 1)}에는 {fmtMs(enhanceDurationMs(2 * CYCLE_LEN - 1))}.
        </LI>
      </UL>
      ...
      <UL>
        <LI>
          다이아 1개 = 남은 시간 {fmtMs(GEM_TO_MS)} 단축. 최대 확률까지 필요한 시간만큼 차감.
        </LI>
        <LI>
          자동 강화는 다이아 단축 → 강화를 정해 둔 예산 또는 조건까지 반복.
          <Fn n={5} />
        </LI>
      </UL>
      <Warn>강화 슬롯을 해제하면 기다린 시간과 넣은 다이아를 날린다.</Warn>
      ...
      />
      <Note>첫 주기 기준. 주기가 오르면 시간은 두 배로 늘고 확률은 그대로.</Note>
      ...
      <FnList
        notes={[
          ...
          '단, 하락 구간에서는 최대 확률까지 채우지 않으면 손해다.',
          '확률 공시에는 메가로 적힌다.',
          '오래 기다리면 유지가 성공으로 바뀔 확률만 오를 뿐, 하락 확률은 그대로다.',
          '화면을 열어 둔 동안만 돈다.',
        ]}
      />
    </>
