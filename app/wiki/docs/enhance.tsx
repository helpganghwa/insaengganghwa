...
export default function Doc() {
  return (
    <>
      <H2 id="flow">진행</H2>
      <P>
        강화에는 돈이 들지 않는다. 시작해 두면 시간이 흐르고, 기다렸다가 카드를 눌러 강화하면
        결과가 나온다. 놔둔다고 저절로 끝나지는 않는다.
      </P>
      <P>
        확률은 기다린 시간에 비례한다. 절반만 기다리고 강화하면 공시 확률의 절반. 게이지가 다 차면
        공시 확률 그대로이고, 그 뒤로는 며칠을 놔둬도 같은 값이다. 급하면 일찍 강화해도 된다.
        확률만 낮아질 뿐 따로 손해 보는 건 없다.
      </P>
      <P>
        강화 칸은 부위당 2개, 세 부위 합쳐 6개다. 장착하지 않은 장비도 강화할 수 있다. 결과가
        나오면 다음 단계 강화가 바로 다시 시작된다.
      </P>
      <Note>
        걸리는 시간과 확률은 시작할 때 값으로 고정된다. 도중에 밸런스가 바뀌어도 진행 중인 시도는
        그대로 간다.
      </Note>
      ...
      <Warn>하락을 막아 주는 보호권 같은 아이템은 없다. 안전 구간을 넘어서면 강화할 때마다 하락이 나올 수 있다.</Warn>
      ...
      <H2 id="gem">다이아 단축</H2>
      <P>
        다이아 1개로 남은 시간을 {fmtMs(GEM_TO_MS)} 줄인다. 단축해도 확률 계산은 똑같다. 남은
        시간을 전부 줄이면 최대 확률로 바로 강화할 수 있다. 필요한 만큼만 차감되고, 환산 비율은
        시작 시점 값으로 고정된다.
      </P>
      <P>
        자동 강화를 켜면 단축과 강화, 재등록을 정해 둔 예산이 닿을 때까지 반복한다. 화면을 열어 둔
        동안만 돈다.
      </P>
