...
export default function Doc() {
  return (
    <>
      <H2 id="piece">계산</H2>
      <P>
        장비 하나의 전투력은 강화 수치로 정해지는 기반 전투력에 초월 보너스를 곱한 값이다. 아이템
        종류에 따른 가중치는 없어서 무기든 장신구든 같은 수치면 같은 값이 나온다.
      </P>
      <P>
        기반 전투력은 강화 수치가 오를수록 증가폭이 커진다. 높은 수치의 장비 하나가 낮은 수치 여럿을
        앞지른다.
      </P>
      <Tbl
      ...
      <P>
        총 전투력은 보유한 장비의 개별 전투력을 전부 더한 값이다. 장착은 계산에 들어가지 않아서 겉모습을
        바꿔도 숫자는 그대로다.
      </P>
      <P>
        보급 상자에서 처음 보는 아이템이 나오면 그 순간 총합이 늘고, 중복이 나와 초월이 오르면 그 장비
        몫이 배수로 커진다.
      </P>
      <Warn>
        총 전투력은 지금 상태를 그대로 따라간다. 강화에서 하락이 뜨면 그 자리에서 함께 내려간다.
      </Warn>
      ...
      <UL>
        ...
        <LI>대난투: 시작 체력과 한 대의 위력이 모두 총 전투력에서 나온다.</LI>
        <LI>
      ...
      <P>
        셋 다 지금 보유 상태를 따라간다. 강화가 떨어지면 순위도 같이 내려간다. 순위표는 서버마다 따로라
        다른 서버 기록과 섞이지 않는다.
      </P>
      <P>
