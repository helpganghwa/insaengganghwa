...
export const meta: WikiDocMeta = {
  ...
  cat: '길드',
  title: '점령전',
  summary: '하루 한 번 붙는 구역 다툼, 집행관과 세금, 방치로 잃는 땅.',
  sections: [
    { id: 'day', label: '진행 시각' },
  ...
  return (
    <>
      ...
      <P>
        ...
        집행관 지정, 거주지 이동이 모두 막힌다. 잠금이 풀린 뒤에 넣은 배치는 그날 밤 전투에 선다.
      </P>
      ...
      <P>
        ...
        {fmtInt(RESIDENCE_SPEEDUP_GEM_PER_MIN)}다이아를 내고 지운다. 강화 세금도 거주지에 쌓여서, 사는
      ...
      <H2 id="deploy">배치</H2>
      <P>
        하루에 설 수 있는 자리는 하나다. 구역을 골라 공격이나 수비로 서고, 다른 곳에 다시 서면 앞의
        배치는 지워진다. 배치를 넣는 것은 본인만 한다.
      </P>
      <UL>
        <LI>수비는 자기 길드가 이미 가진 구역에만 선다.</LI>
        <LI>공격은 자기 길드 것이 아닌 구역에 서되, 자기 길드 구역과 맞닿아 있어야 한다.</LI>
        <LI>가진 구역이 하나도 없는 길드는 어디든 첫 상륙이 된다.</LI>
      ...
      <P>
        구역마다 한 명을 세운다. 집행관 지정 권한자가 그 구역에 사는 길드원 중에서 고르고, 한 사람이
      ...
      <P>
        많이 가진 길드일수록 더 걷는다. 소유 구역 하나당 {bpPct(GUILD_ZONE_TAX_BONUS * 10_000)},{' '}
        {fmtInt(REGION_COUNT)}개 지역 중 하나를 통째로 가지면 그 지역마다{' '}
        {bpPct(GUILD_FULL_REGION_TAX_BONUS * 10_000)}가 더 붙고, 이 배율이 그 길드의 모든 구역 누적에
        곱해진다. 구역을 잃으면 배율도 바로 내려간다.
      </P>
      <P>
      ...
      <P>
        ...
        {bpPct((1 - GUILD_EXECUTOR_TAX_CUT) * 10_000)}는 길드 세금 풀에 모인다.
      </P>
      <Note>
        풀은 세금 분배 권한자가 연다. 균등 · 기여 비례 · 오늘 기부자 · 직접 중에서 방식을 고르면 금액이
        채워지고, 사람별 금액은 그 자리에서 고친다. 지급은 보상 우편으로 간다.
      </Note>

      <H2 id="abandon">방치 페널티</H2>
      <P>
        그날 전투에 공격도 수비도 한 명 없고 집행관 자리까지 비어 있으면 그 구역은 방치가 된다. 남이
        노리지 않았어도 마찬가지다.
      </P>
      <Warn>
      ...
      <P>
        ...
        다만 그 전투일이 시작된 뒤에 점령한 구역은 방치로 치지 않는다. 길드가 해산할 때도 보유 구역은
        중립이 된다.
      </P>

      <H2 id="chronicle">연대기</H2>
      <P>
        하루치 점령전은 자정에 한 편의 이야기로 묶여 세계지도 아래에 열린다. 어느 길드가 어디를 노렸고
        누가 막아냈는지가 그날 기록으로 남는다.
      </P>
      <P>
        점령도 방치 상실도 해산도 눈에 띄는 활약도 없는 날은 아예 기록하지 않는다. 판이 크게 바뀐
        날에만 한 줄 제목이 붙고, 그런 날짜만 연표에 쌓인다.
      </P>
      <P>
