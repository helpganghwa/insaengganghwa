  ...
  return (
    <>
      ...
      <UL>
        ...
        <LI>
          길드원끼리 자기 길드 구역에 모여 거주하는 것이 좋다. 세금이 그대로 길드 수입이 된다.
        </LI>
      </UL>
      <Warn>
      ...
      <UL>
        ...
        <LI>
          수비는 자기 <DocLink slug="guild">길드</DocLink>가 이미 점령한 구역에만 배치한다.
        </LI>
        <LI>
      ...
      <H2 id="battle">전투</H2>
      <UL>
        <LI>전투는 공격 배치가 하나라도 들어온 구역에서만 진행된다.</LI>
      ...
      <UL>
        <LI>체력은 유효 전투력의 {fmtInt(CONQUEST_HP_MULT)}배다.</LI>
        <LI>
          한 번의 공격으로 들어가는 피해는 공격하는 쪽 유효 전투력의{' '}
          {bpPct(CONQUEST_DMG_MIN * 10_000)}에서 {bpPct(CONQUEST_DMG_MAX * 10_000)} 사이다.
        </LI>
        <LI>같은 길드끼리는 전투하지 않으며, 한 길드만 남는 순간 그 길드가 승리한다.</LI>
        <LI>
          승리한 길드가 원래 소유 길드와 다르면 자정에 구역 소유권이 넘어간다. 끝까지 승자가 갈리지
          않으면 소유 길드는 그대로다.
        </LI>
      ...
      <UL>
        <LI>
          구역마다 한 명이며, 집행관 지정 권한자가 그 구역에 거주하는 길드원 중에서 지정한다.
          <Fn n={4} />
        </LI>
        <LI>
          집행관은 배치하지 않아도 그 구역 수비로 참여하며, 유효 전투력이{' '}
      ...
      <UL>
        ...
        <LI>
          ...
          첫 수금이 가능하고, 그 뒤로도 {fmtMs(TAX_COLLECT_COOLDOWN_MIN * 60_000)}에 한 번이다.
          <Fn n={6} />
        </LI>
        <LI>
      ...
      <UL>
        <LI>
          그날 전투에 공격도 수비도 한 명 없고 집행관 자리까지 비어 있으면 그 구역은 방치로
          처리된다. 공격이 들어오지 않은 날도 마찬가지다.
          <Fn n={7} />
        </LI>
        <LI>배치할 사람이 없는 날은 집행관만 세워 둬도 방치는 면한다.</LI>
        <LI>중립이 된 구역은 누구나 공격할 수 있어, 오래 방치될수록 소유 길드가 자주 바뀐다.</LI>
      </UL>
      <Warn>
        방치된 구역은 자정 발표 직후 중립으로 풀린다. 되찾으려면 다시 공격해야 한다. 쌓여 있던 세금은
        구역에 남아 다음 소유 길드가 수금한다.
      </Warn>

      <H2 id="chronicle">연대기</H2>
      <UL>
        <LI>하루치 점령전은 자정에 한 편의 이야기로 묶여 세계지도 아래에 발표된다.</LI>
        <LI>
      ...
      <FnList
        notes={[
          '잠금이 풀린 뒤에 넣은 배치는 그날 밤 전투에 반영된다.',
          '다른 곳에 다시 배치하면 앞의 배치는 해제된다. 거주하지 않는 구역을 고르면 거주지 이동을 함께 확인한다.',
          '점령한 구역이 하나도 없는 길드는 인접 조건 없이 아무 구역이나 공격할 수 있다.',
          '한 사람이 두 구역을 맡지는 못한다. 새로 점령한 구역은 집행관이 빈 채로 시작한다.',
          '집행관이 어딘가에 배치하면 집행관 자리도 해제된다. 수금하지 않은 세금은 구역에 그대로 남는다.',
          '빼앗은 구역도 처음부터 다시 센다.',
          '그날 새로 점령한 구역은 방치로 처리하지 않는다.',
