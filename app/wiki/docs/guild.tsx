...
export default function Doc() {
  return (
    <>
      <H2 id="create">결성</H2>
      <P>
        결성 비용은 {fmtInt(GUILD_CREATE_COST_DIAMOND)}다이아다. 한 서버에 소속할 수 있는 길드는
        하나라 이미 어디 들어가 있으면 못 만든다. 만든 사람이 길드장이 되고 가입 방식은 승인제로
        시작한다.
      </P>
      <UL>
        <LI>
        ...
        <LI>이름은 전 서버를 통틀어 하나라, 다른 서버에 같은 이름이 있으면 막힌다.</LI>
        <LI>
          문양은 결성할 때 한 장이 무료다. 이후로는 만들 때마다{' '}
      ...
      <H2 id="join">가입</H2>
      <P>
        가입 방식은 자유 가입과 승인제 둘이다. 자유 가입이면 누르는 즉시 들어가고, 승인제면 신청이
        걸린 뒤 가입 관리 권한자가 승인하거나 거절한다. 어느 쪽이든 정원이 차 있으면 못 들어간다.
      </P>
      <P>
        신청은 한 번에 한 곳만 걸린다. 다른 길드에 신청하면 앞의 신청이 그리로 옮겨 가고,{' '}
        {fmtInt(GUILD_JOIN_REQUEST_TTL_DAYS)}일이 지나면 사라진다. 어디든 들어가면 남은 신청은
        정리된다.
      </P>

      <H2 id="donate">기부</H2>
      <P>
        기부는 하루 {fmtInt(GUILD_DONATIONS_PER_DAY)}번, 한국 시간 자정에 다시 찬다. 단계가 올라갈수록
        비용이 오른다({DONATION_COSTS}). 얻는 경험치는 단계와 상관없이{' '}
        {fmtInt(GUILD_DONATION_TIERS[0].xp)}으로 같아서, 뒤 단계는 같은 것을 비싸게 사는 셈이다.
      </P>
      <P>
        기부 한 번은 길드 경험치와 개인 기여도에 같은 값으로 쌓인다. 기여도는 길드 안에 누적으로
        남고, 세금 분배와 길드장 자동 위임에서 기준이 된다.
      </P>

      <H2 id="level">레벨과 정원</H2>
      <P>
        레벨은 기부로만 오른다. 다음 레벨까지 필요한 경험치가 {fmtInt(GUILD_XP_PER_LEVEL_STEP)}씩
        늘어나서 뒤로 갈수록 한 레벨에 걸리는 날이 길어진다.
      </P>
      <P>
        정원은 {fmtInt(GUILD_BASE_CAPACITY)}명에서 시작해 레벨 하나당 한 자리씩 늘고{' '}
        {fmtInt(GUILD_MAX_CAPACITY)}명에서 멈춘다. 정원이 멈춘 뒤에도 레벨은 계속 오르고, 길드 순위의
        기준으로 남는다.
      </P>
      ...
      <H2 id="leave">탈퇴</H2>
      <P>
        길드원은 언제든 나간다. 나가는 순간 맡고 있던 집행관 자리와 넣어둔 배치가 함께 풀린다.
      </P>
      <Warn>
      ...
      <P>
        길드장은 길드원이 남아 있는 동안 탈퇴가 막힌다. 위임하거나 해산해야 나간다. 혼자 남은
        상태에서 탈퇴하면 그대로 해산된다.
      </P>

      <H2 id="disband">해산</H2>
      <P>해산은 길드장만 한다. 길드원 동의를 묻지 않고 미리 알림도 가지 않는다.</P>
      <Warn>
        해산하면 보유 구역이 전부 중립으로 풀리고 모아둔 세금은 사라진다. 문양과 길드 기록도 함께 지워지고
        되돌릴 방법은 없다.
      </Warn>
      <P>
        길드장이 오래 접속하지 않고 위임받을 사람도 없으면 길드는 자동으로 해산된다. 이때는
        전원에게 해산 우편이 간다. 자세한 조건은 <DocLink slug="guild-roles">길드 권한</DocLink>에 있다.
      </P>
      <P>
