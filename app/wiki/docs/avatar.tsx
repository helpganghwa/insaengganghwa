...
export default function Doc() {
  return (
    <>
      ...
      <H2 id="return">반환</H2>
      <UL>
        <LI>아바타 관리에서 아바타를 선택하고 반환을 누르면 그 아바타는 즉시 회수된다.</LI>
        <LI>운영자가 검토 후 다이아를 우편으로 지급한다. 뚜렷한 문제가 있었다면 생성에 쓴 다이아 전액, 그 밖의 경우는 절반.</LI>
        <LI>기준 금액은 그 아바타를 만들 때 실제로 낸 다이아다. 첫 생성 할인가로 만든 아바타는 할인가 기준.</LI>
        <LI>기본 아바타, 신고 처리 중인 아바타는 반환할 수 없다.</LI>
      </UL>
