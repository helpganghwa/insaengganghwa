...
export default function Doc() {
  return (
    <>
      ...
      <P>
        다이아는 요청과 동시에 빠진다. 그림은 십 분 안팎이면 나오고, 생성 자리가 다 차 있으면 대기
        순서와 예상 시간이 뜬다. 자리는 {fmtMs(PROFILE_GEN_SLOT_MINUTES * 60_000)}에 하나씩 비는
        기준이라 붐빌 때는 더 걸린다.
      </P>

      <H2 id="review">검토</H2>
      <P>
        그림이 나오면 자동 검토를 거치고, 통과한 것만 아바타 목록에 들어온다. 검토는 안전과 형태만
        본다. 색이나 체형, 화풍이 생각과 다른 것은 떨어지는 사유가 아니다.
      </P>
      <Tbl
        head={['항목', '떨어지는 경우']}
        rows={[
      ...
      />
      <P>
        애매하면 통과시킨다. 각도 때문에 팔 하나가 가려지거나 무기가 망토 뒤로 숨은 정도는 넘어간다.
      </P>

      <H2 id="refund">환불</H2>
      <P>
        검토에서 떨어지면 다이아를 전액 돌려주고 우편으로 사유를 보낸다. 돌려받은 다이아로 곧바로
        다시 만들면 된다.
      </P>
      <UL>
        <LI>검토에서 떨어지면 전액 환불.</LI>
        <LI>생성 도중 오류가 나면 전액 환불.</LI>
        <LI>그림이 오래 안 나오거나 대기가 지나치게 길어지면 전액 환불.</LI>
      </UL>
      <Warn>
      ...
      <P>
        ...
        되고, 헤더 썸네일과 프로필, 공유 카드, 채팅 미니 프로필에 그 아바타가 뜬다.
      </P>
      <UL>
