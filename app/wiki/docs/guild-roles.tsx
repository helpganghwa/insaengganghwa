  ...
  return (
    <>
      ...
      <P>
        ...
        {fmtInt(GUILD_MAX_VICE)}명까지 두고, 할 수 있는 일은 길드장이 사람마다 허용한 만큼이다.
        길드원은 권한을 갖지 않는다.
      </P>
      <P>
        권한은 부길드장 자리에 붙는다. 부길드장을 해제하면 허용한 권한도 함께 사라지고, 다시 임명해도
        기본값에서 다시 시작한다.
      </P>

      <H2 id="perms">부길드장 권한</H2>
      <P>
        허용할 수 있는 권한은 {fmtInt(GUILD_PERM_ORDER.length)}가지이고 하나씩 따로 켜고 끈다. 마지막
        칸은 갓 임명한 부길드장이 기본으로 갖는 상태이고, 길드장은 이것도 끌 수 있다.
      </P>
      <Tbl head={['권한', '내용', '임명 직후']} rows={PERM_ROWS} />
      <UL>
        <LI>
          {GUILD_PERM_META.deploy.label}는 남의 배치를 물리는 것뿐이다. 배치를 넣는 것은 언제나
          본인이다.
        </LI>
        <LI>{GUILD_PERM_META.executor.label}에는 그 구역 세금 수금 권한이 따라간다.</LI>
        <LI>
      ...
      <P>
        ...
        한다. 추방된 사람이 맡고 있던 집행관 자리와 넣어둔 배치는 함께 풀린다.
      </P>
      <Warn>
      ...
      <P>
        ...
        부길드장이 먼저고, 그다음은 기여도 순이다. 자리를 넘긴 전 길드장은 길드원으로 남는다.
      </P>
      <Warn>
        넘겨받을 길드원이 하나도 없으면 위임 대신 길드가 해산된다. 보유 구역은 중립이 되고 모아둔
        세금은 사라진다. 전원에게 해산 우편이 간다.
      </Warn>
      <P>
