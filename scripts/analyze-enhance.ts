 ...
 *  - 흡수벽: ℓ=99 (도달 시 종료)
 *  - 시도 시간 d(ℓ) = baseAttemptDurationMs(ℓ) × 2^cycle
 *
...
function computeCycleZero() {
  ...
  const d = new Array(CYCLE_LEN).fill(0).map((_, l) => baseAttemptDurationMs(l));
...
function main() {
  ...
  console.log('\n=== 검증 (BALANCE §1.1 앵커) ===');
  const target30 = 24 * 3600_000;
  const target50 = 3 * 24 * 3600_000;
  const target99 = 28 * 24 * 3600_000;
