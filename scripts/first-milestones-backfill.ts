// 최초 이정표 소급(2026-09-26, docs/TITLES.md "최초 이정표 칭호") — 배포 시점에 이미 임계를 넘긴 유저를 **실제 넘은 순서대로**
...
//   순서: 0218 적용 → 이 스크립트 --apply → 코드 배포. 프로덕션은 URL을 명시(PROD_DATABASE_URL 값).
//   전체가 한 트랜잭션 — dry-run은 `read only`(세션 SET 금지 — 트랜잭션 풀러로 새어 다른 요청을 막는다), apply는 전부 넣거나 전부 안 넣는다.
// 순서 복원:
...
// main()이 트랜잭션 안에서 채운다 — 아래 조회·기록은 모두 이 핸들로.
let sql: postgres.TransactionSql = undefined as unknown as postgres.TransactionSql;
...
async function main(): Promise<void> {
  await pg.begin(apply ? 'read write' : 'read only', async (tx) => {
    sql = tx;
    await body();
  });
  await pg.end();
...
async function body(): Promise<void> {
  ...
  if (apply && !hasTable) throw new Error('milestone_firsts 없음 — 0218을 먼저 적용');
  console.log(`[first-backfill] ${apply ? 'APPLY' : 'dry-run'} · 서버 ${servers.join(',')}${hasTable ? '' : ' · (0218 미적용: 기존 행 검사 생략)'}`);
  let planned = 0;
      ...
      if (apply) {
        {
          for (let i = 0; i < hits.length; i++) {
            await sql`insert into milestone_firsts (server_id, milestone, rank, user_id, reached_at) values (${serverId}, ${m.key}, ${i + 1}, ${hits[i]!.userId}::uuid, ${hits[i]!.at})`;
          }
        }
