 ...
 * 닉네임이 자리를 잡고 있으므로 닉네임 예약 로직이 필요 없고, 오픈 첫날 월드가 비어 보이지 않는다.
 *
 * 이월 범위 = **닉네임 + 감사/초대 보상뿐**. 그 외는 전부 초기화된다.
 * ⚠ 아바타는 이월하지 않는다(기획 삭제, 2026-07-31 확정) — 누구든 기본 2종으로 시작한다.
 *   cbt_carryover 스키마에도 아바타 컬럼이 없어 되살릴 원본 자체가 남지 않는다.
 *
 ...
 *  2. 기본 아바타 2종만 생성, 그중 랜덤 1개를 active로.
