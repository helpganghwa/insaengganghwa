 ...
 *  - 6단 팔레트 포스터라이즈 + Bayer 4×4 디더 — 맵 픽셀아트와 톤 일치
 *  - 한랭 남색 계열(다크 UI 무드) — 짙은 골과 밝은 마루의 명암으로 입체감
...
// 심연(골) → 구름 마루. 다크 UI(zinc-950)와 어울리는 한랭 남색 계열.
...
for (let y = 0; y < N; y++) {
  for (let x = 0; x < N; x++) {
    ...
    const t = Math.min(0.999, Math.max(0, d + BAYER[y % 4]![x % 4]! * (1 / (RAMP.length - 1)) * 0.55));
